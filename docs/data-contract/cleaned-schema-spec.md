...
This document defines the V1 `@fairy/data` cleaned-data contract after the
...
contracts for `@fairy/data`, `@fairy/core`, and `@fairy/cli`.
...
`data/cleaned/` is the repo-level staging directory. `packages/data/cleaned/` is
the `@fairy/data` package payload mirror generated by `sync-cleaned`.
...
| Export | Purpose |
|---|---|
| `@fairy/data/cleaned` | Total cleaned-data entry. |
| `@fairy/data/cleaned/<domain>` | Domain entry, for example `@fairy/data/cleaned/deadly-assault`. |
| `@fairy/data/types` | TypeScript types for cleaned data and source metadata. |
| `@fairy/data/cleaned/i18n/<domain>` | Game-label i18n resources, for example `deadly-assault`. |
...
- package dry-run excludes `data/source`, raw HTML/JS/JSON, GPL license copies,
  `docs/reference`, Excel, and tests;
- `verify:buhflipexplode-da` passes offline;
