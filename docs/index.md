...
- [静态构筑解析系统路线图](./specs/static-build-resolver-roadmap.md) — 当前执行状态：已推进到 V34；当前 contract 已覆盖 source views、trigger matrix、utility views、unified source-entry collection、main resolver summary、core skill matrix summary 与 core skill matrix effect summary；`V34` 正在冻结 matrix row damage summary
...
- [静态构筑解析系统 V34](./specs/static-build-resolver-v34.md) — 当前阶段：已冻结 matrix row damage-summary contract；目标是为 `ResolveStaticBuildSkillMatrixRow` 增加稳定 `damageSummary`
