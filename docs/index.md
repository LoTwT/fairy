...
- [静态构筑解析系统路线图](./specs/static-build-resolver-roadmap.md) — 当前执行状态以 roadmap 为准；当前 contract 已覆盖 single-build resolver、skill matrix、trigger matrix、source damage views、source utility views、unified source-entry collection，以及对应 compact/helper exports，并持续推进公开 contract 的显式化与文档收口。
...
- [静态构筑解析系统 V369](./specs/static-build-resolver-v369.md) — 当前阶段：已收口；`resolve-build-schemas.ts` 的公开名称、attribute、combat-tag、anomaly-type 字段已统一复用显式 alias
- [静态构筑解析系统 V370](./specs/static-build-resolver-v370.md) — 当前阶段：已收口；`resolve-build-schemas.ts` 的公开 snapshot 与场景布尔/数值输入已统一复用显式 alias
...
- [静态构筑解析系统 V492](./specs/static-build-resolver-v492.md) — 当前阶段：已收口；`resolve-build-schemas.ts` 中 `scenario.damageType` 的 branch literal schema 已统一复用共享 schema 常量
- [静态构筑解析系统 V493](./specs/static-build-resolver-v493.md) — 当前阶段：已收口；`resolve-build-schemas.ts` 中 `scenario` multiplier 的共享 `number|string` scalar schema 已统一复用共享 schema 常量
