...
- [静态构筑解析系统路线图](./specs/static-build-resolver-roadmap.md) — 当前执行状态：已推进到 V33；当前 contract 已覆盖 source views、trigger matrix、utility views、unified source-entry collection、main resolver summary、core skill matrix summary 与 core skill matrix effect summary；`V33` 正在冻结 support-scope normalization
...
- [静态构筑解析系统 V33](./specs/static-build-resolver-v33.md) — 当前阶段：已冻结 support-scope normalization；目标是统一 `resolve-build-*` 高层 tool 的 unsupported / support-scope 组装与 catalog helper
