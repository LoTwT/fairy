...
- [静态构筑解析系统 V49](./specs/static-build-resolver-v49.md) — 当前阶段：已完成 scope freeze；目标是为 `skill matrix row` 增加稳定 `summary`
