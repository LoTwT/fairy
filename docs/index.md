...
- [静态构筑解析系统路线图](./specs/static-build-resolver-roadmap.md) — 当前执行状态：已推进到 V36；当前 contract 已覆盖 source views、trigger matrix、utility views、unified source-entry collection、main resolver summary、core skill matrix summary、core skill matrix effect summary、matrix row damage summary 与 matrix row compact fields；`V36` 正在冻结 matrix row explanation contract
...
- [静态构筑解析系统 V36](./specs/static-build-resolver-v36.md) — 当前阶段：已冻结 matrix row explanation contract；目标是为 `ResolveStaticBuildSkillMatrixRow` 增加稳定的 `diagnostics / sourceNotes`
