...
- [静态构筑解析系统路线图](./specs/static-build-resolver-roadmap.md) — 当前执行状态：已推进到 V31；当前 contract 已覆盖 source views、trigger matrix、utility views、unified source-entry collection、main resolver summary 与 core skill matrix summary，`V31` 正在冻结 core skill matrix effect summary
...
- [静态构筑解析系统 V31](./specs/static-build-resolver-v31.md) — 当前阶段：已冻结 core skill matrix effect-summary contract；目标是为 `ResolveStaticBuildSkillMatrixResult` 增加稳定 `effectSummary`，把当前只存在于高层 tool 的增益清单聚合逻辑下沉到 `zzz-data`
