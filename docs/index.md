...
- [静态构筑解析系统路线图](./specs/static-build-resolver-roadmap.md) — 当前执行状态：已推进到 V25；当前 contract 已覆盖 source views、trigger matrix、utility views 与 unified source-entry collection，异常/紊乱 skill matrix 仍为显式 out-of-scope
...
- [静态构筑解析系统 V25](./specs/static-build-resolver-v25.md) — 当前阶段：已完成 scope freeze；目标是扩第二批 utility / resource view，收录 `时光切片` 的 `喧响值 + 能量` 结构化条目，并把 utility-only 查询从 damage-agent catalog 解耦
