...
- [静态构筑解析系统 V51](./specs/static-build-resolver-v51.md) — 当前阶段：已完成 scope freeze；目标是为 `trigger-matrix row` 增加稳定 `summary`
