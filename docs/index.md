...
当前仓库统一通过 `packages/zzz-data/scripts/sources` 管理四个数据源的同步：`xlsx` 负责 `.sources/source.xlsx -> data/source/xlsx/zh-CN` 与 `scripts/sources/xlsx/types` 产物更新，`gachabase` / `buhflipexplode` / `mihoyo-wiki` 负责远端 source 数据抓取与派生快照生成。`packages/zzz-data/.sources/source.xlsx` 是手动下载的本地输入，不纳入版本管理；`.sources/source.xlsx.metadata.json` 记录最近一次成功处理的哈希与时间。
