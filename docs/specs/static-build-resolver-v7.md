...
- `V7.3` source adoption：前三批已完成（`柏妮思 M6` 的 `25% 火抗无视 -> resolvedSnapshot.bucketDeltas.ignoreResistance`；`格莉丝 M2`、`简`、`派派`、`时流贤者` 的异常积蓄效率 / 异常倍率折算 -> `resolvedSnapshot.multiplierFactors.skillMultiplierFactor`）
