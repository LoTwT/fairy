...
- [共享来源规范](source.md)：版本选择、安全 HTTP、实体发现、轻量输入检查、本地缓存、包边界和自动化验证范围。
- [来源数据整合规范](../data/integration.md)：代理人结构校验、多语言拆分、正式类型、离线全量构建及验收边界；实现状态由该规范维护。
...
“已调研，可抓取”表示端点、ID 发现方式及代表性结构已经由真实上游数据确认，并已登记到通用抓取器；该状态只描述抓取能力，不承诺抓取器执行运行时 schema、字段级验证、跨实体验证或生成可复现快照。代理人整合器的独立验证范围见上方整合规范。
