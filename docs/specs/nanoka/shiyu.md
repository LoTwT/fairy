...
- `3.0` 定向加入 Shiyu 后发布六实体组合快照，共 1,131 个资源；五个未选实体的 1,017 个资源均以 carried-forward 参与 staging 验证；
- 重复定向抓取中 Shiyu 摘要及 112 份详情与上游 manifest 共 114 个资源均通过 HTTP 304 复用；
- `3.1.5+17516165` 和 `3.1.12+17625891` 的六实体全量快照均包含 1,161 个资源；
- 三版本离线 verify 均通过，`shiyu-monster-reference/v1` 分别重算 5,462、5,710、5,710 个 `zh/en` Monster 引用且未解析数为零；
- 测试副本中的 Shiyu Monster ID 篡改被文件、实体及跨实体分层验证拒绝；
- 默认自动化测试使用 mock fetch 和临时目录，不访问 Nanoka，raw cache 与空公共 API 边界保持不变。

上述资源和引用计数是本次验收证据，不是永久阈值。
