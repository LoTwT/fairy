...
- 独立的[来源整合器](../data/integration.md#离线全量构建的共用能力)已支持明确本地版本的离线全量新制品构建与验证；不改变抓取缓存语义
- 适用包：`@randomplay/data`
...
2026-09-18，接入 WEngine 生产类别并提交三类别真实快照前，按本节要求重新评审一次：

- **上游政策**：`https://zzz.nanoka.cc/` 当日返回 200，页面仍声明 `<meta name="robots" content="index, follow">`，
  未发现使用条款、再分发授权或禁止性声明；`https://static.nanoka.cc/manifest.json` 匿名可访问（当日 `live` 为
  3.2、`latest` 为 3.3.2+18921567，`available` 仍包含本次使用的 3.1），与登记的 `contentSignals` 一致。
  结论与 2026-09-17 相同，不把公开可访问当作再分发授权。本次仍使用现有 3.1 缓存，未联网覆盖缓存或升级来源版本。
- **存储成本**：新增 `w-engines` 类别后，Git 跟踪的 integrated 由 265 个文件增加到 550 个（新增 95 名成员 ×
  3 个文件）；w-engines 目录实际字节约 1.05 MiB，agents 约 9.12 MiB、drive-discs 约 0.04 MiB，合计约 10.59 MiB；
  npm 包发布副本相应增加这 285 个 JSON（本次打包验收解包 11,362,369 字节、550 个 JSON）。raw 仍被 Git 精确忽略，
  不进入 Git 或 npm，成本评估不依赖 raw 入库。
- **制品契约**：提交的数据仅限 `packages/data/integrated/` 下的已验证 v3 制品（索引与成员 JSON）；
...
2026-09-19，接入 Monster 生产类别并提交五类别真实快照前，按本节要求重新评审一次：
...
- **上游政策**：`https://zzz.nanoka.cc/` 当日返回 200，页面仍声明 `<meta name="robots" content="index, follow">`
  ...
  `available` 仍包含本次使用的 3.1），与登记的 `contentSignals` 一致。结论与 2026-09-17 相同，不把公开可访问
  当作再分发授权。本次仍使用现有 3.1 缓存，未联网覆盖缓存或升级来源版本。
- **存储成本**：新增 `monsters` 类别后，Git 跟踪的 integrated 由 676 个文件增加到 1555 个（新增 293 名成员 ×
  3 个文件）；monsters 目录实际字节约 6.07 MiB，agents 约 9.41 MiB、w-engines 约 1.57 MiB、bangboos 约
  1.29 MiB、drive-discs 约 0.35 MiB；npm 包发布副本相应增加这 879 个 JSON。raw 仍被 Git 精确忽略，
  不进入 Git 或 npm，成本评估不依赖 raw 入库。
- **制品契约**：提交的数据仅限 `packages/data/integrated/` 下的已验证 v3 制品（索引与成员 JSON）；
  抓取器、管理记录、维护报告与 raw 不入库。结论：在现行边界内，接入 Monster 与既有类别分发使用同一契约，
  无新增阻碍；若未来扩大再分发范围（例如整包镜像），仍须按本节重新评审。

2026-09-19，接入 Shiyu 生产类别并提交六类别真实快照前，按本节要求重新评审一次：

- **上游政策**：`https://zzz.nanoka.cc/` 当日返回 200，页面仍声明 `<meta name="robots" content="index, follow">`，
  未发现使用条款、再分发授权或禁止性声明；`https://static.nanoka.cc/manifest.json` 匿名可访问（当日 `live`
  为 3.2、`latest` 为 3.3.2+18921567，`available` 仍包含本次使用的 3.1）。结论与 2026-09-17 相同，不把公开
  可访问当作再分发授权。本次仍使用现有 3.1 缓存，未联网覆盖缓存或升级来源版本。
- **存储成本**：新增 `shiyu` 类别后，Git 跟踪的 integrated 由 1555 个文件增加到 1732 个（新增 59 名成员 ×
  3 个文件）；shiyu 目录实际字节约 5.08 MiB，monsters 约 6.07 MiB、agents 约 9.41 MiB、w-engines 约
  1.57 MiB、bangboos 约 1.29 MiB、drive-discs 约 0.35 MiB；npm 包发布副本相应增加这 177 个 JSON。raw 仍被
  Git 精确忽略，不进入 Git 或 npm，成本评估不依赖 raw 入库。
- **制品契约**：提交的数据仅限 `packages/data/integrated/` 下的已验证 v3 制品（索引与成员 JSON）；
  抓取器、管理记录、维护报告与 raw 不入库。结论：在现行边界内，接入 Shiyu 与既有类别分发使用同一契约，
  无新增阻碍；若未来扩大再分发范围（例如整包镜像），仍须按本节重新评审。

2026-09-19，接入 Boss 生产类别并提交七类别真实快照前，按本节要求重新评审一次：

- **上游政策**：`https://zzz.nanoka.cc/` 当日返回 200，页面仍声明 `<meta name="robots" content="index, follow">`，
  未发现使用条款、再分发授权或禁止性声明；`https://static.nanoka.cc/manifest.json` 匿名可访问（当日 `live`
  为 3.2、`latest` 为 3.3.2+18921567，`available` 仍包含本次使用的 3.1）。结论与 2026-09-17 相同，不把公开
  可访问当作再分发授权。本次仍使用现有 3.1 缓存，未联网覆盖缓存或升级来源版本。
- **存储成本**：新增 `boss` 类别后，Git 跟踪的 integrated 由 1732 个文件增加到 1864 个（新增 44 名成员 ×
  3 个文件）；boss 目录实际字节约 1.84 MiB，shiyu 约 5.08 MiB、monsters 约 6.07 MiB、agents 约 9.41 MiB、
  w-engines 约 1.57 MiB、bangboos 约 1.29 MiB、drive-discs 约 0.35 MiB；npm 包发布副本相应增加这 132 个
  JSON。raw 仍被 Git 精确忽略，不进入 Git 或 npm，成本评估不依赖 raw 入库。
- **制品契约**：提交的数据仅限 `packages/data/integrated/` 下的已验证 v3 制品（索引与成员 JSON）；
  抓取器、管理记录、维护报告与 raw 不入库。结论：在现行边界内，接入 Boss 与既有类别分发使用同一契约，
  无新增阻碍；若未来扩大再分发范围（例如整包镜像），仍须按本节重新评审。

2026-09-19，接入 Simul 生产类别并提交八类别真实快照前，按本节要求重新评审一次：

- **上游政策**：`https://zzz.nanoka.cc/` 当日返回 200，页面仍声明 `<meta name="robots" content="index, follow">`，
  未发现使用条款、再分发授权或禁止性声明；`https://static.nanoka.cc/manifest.json` 匿名可访问（当日 `live`
  为 3.2、`latest` 为 3.3.2+18921567，`available` 仍包含本次使用的 3.1）。结论与 2026-09-17 相同，不把公开
  可访问当作再分发授权。本次仍使用现有 3.1 缓存，未联网覆盖缓存或升级来源版本。
- **存储成本**：新增 `simul` 类别后，Git 跟踪的 integrated 由 1864 个文件增加到 1873 个（新增 3 名成员 ×
  3 个文件）；simul 目录实际字节约 0.75 MiB，boss 约 1.84 MiB、shiyu 约 5.08 MiB、monsters 约 6.07 MiB、
  agents 约 9.41 MiB、w-engines 约 1.57 MiB、bangboos 约 1.29 MiB、drive-discs 约 0.35 MiB；npm 包发布副本
  相应增加这 9 个 JSON。raw 仍被 Git 精确忽略，不进入 Git 或 npm，成本评估不依赖 raw 入库。
- **制品契约**：提交的数据仅限 `packages/data/integrated/` 下的已验证 v3 制品（索引与成员 JSON）；
  抓取器、管理记录、维护报告与 raw 不入库。结论：在现行边界内，接入 Simul 与既有类别分发使用同一契约，
  无新增阻碍；若未来扩大再分发范围（例如整包镜像），仍须按本节重新评审。
