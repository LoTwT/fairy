...
- 下一实现范围：Simul adapter 与图结构验证
- 验证状态：自动化检查通过，Agents、Drive Discs、W-Engines、Bangboos、Monsters 与 Shiyu 已完成实际在线组合快照验收；Simul 和 Boss 已完成三版本全量结构调研和正式建模，尚未实现
- 适用包：`@randomplay/data`
...
随着实体注册表扩展，历史 `v2` 快照可以使用代码中显式冻结登记的历史实体集合 epoch。当前冻结的历史 epoch 为 `character, equipment`、`character, equipment, weapon`、`character, equipment, weapon, bangboo` 和 `character, equipment, weapon, bangboo, monster`。历史 epoch 必须是曾由本工具发布的完整实体集合，不得接受当前注册表的任意子集；清单声明、资产集合、summary 和 validation 必须在该 epoch 内严格闭合。正常新发布仍必须包含当前全部启用实体。定向新增实体时，可以从包含全部未选实体的合法历史 epoch 构建当前完整 staging，并且只有完整 staging 验证成功后才升级磁盘快照。
