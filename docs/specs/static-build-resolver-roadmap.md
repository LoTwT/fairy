...
## 477. V474 resolve-build dynamic snapshot object contracts
...
## 478. V475 resolve-build state snapshot object contracts

### 目标

`V475` 只解决一件事：

- 把 `zzz-agent` `resolve-build-schemas.ts` 中 `stateSnapshot` 的匿名嵌套对象 contract 收口为显式 interface。

### 范围

1. `BuildToolStateSnapshotFlags`
2. `BuildToolStateSnapshotValues`
3. `BuildToolStateSnapshotInput.flags / values`
...
- `V475.1` 已完成：范围冻结到 `stateSnapshot` 的嵌套对象 contract
- `V475.2` 已完成：相关字段已统一复用显式 interface

## 479. V476 resolve-build resolved snapshot object contracts
...
## 480. V477 resolve-build effect override contracts
...
## 481. V478 resolve-build dynamic snapshot nested schemas
...
## 482. V479 resolve-build state snapshot nested schemas
...
## 483. V480 resolve-build resolved snapshot nested schemas
...
## 484. V481 resolve-build combat tag list schema
...
## 485. V482 resolve-build top-level input scalar schemas
...
## 486. V483 resolve-build enemy scalar schemas
...
## 487. V484 resolve-build final panel scalar schemas
...
## 488. V485 resolve-build scenario scalar schemas
...
## 489. V486 resolve-build dynamic snapshot scalar schemas
...
## 490. V487 resolve-build state snapshot scalar schemas
...
## 491. V488 resolve-build resolved snapshot scalar schemas
...
## 492. V489 resolve-build drive-disc set scalar schemas
...
## 493. V490 resolve-build effect override scalar schemas
...
## 494. V491 resolve-build combat tag element schema
...
## 495. V492 resolve-build scenario damage-type literal schemas
...
## 496. V493 resolve-build scenario multiplier scalar schemas
