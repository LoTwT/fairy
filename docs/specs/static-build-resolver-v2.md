...
V2 当前通过公开 `data/zh-CN/w-engines.json` 动态支持**全部强攻 / 命破音擎**，并要求与代理人 specialty 兼容。当前共 `31` 把。

约定：

- 音擎支持范围跟随当前发布数据中的强攻 / 命破音擎动态变化
- 代理人与音擎的兼容关系按 `specialty` 校验，而不是按 `exclusiveWeapon` 绑定
