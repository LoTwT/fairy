...
10. `V10` source-specific delta view：已完成 `V10.2` 首批 delta source coverage；`爱芮 [异放]` 已通过“含快照结果 - 去除快照结果”的独立 view 暴露，`霰落星殿` 与 `混沌重金属 4件` 先保留为 research-only，见 [静态构筑解析系统 V10](./static-build-resolver-v10.md)
