...
296. `V296` build entry and row id contracts：当前阶段进行中；当前目标是为 source-note / source-view / matrix row 相关公开 contract 中的通用 `id` 补显式公开 type，并统一复用，不改变任何运行时行为，见 [静态构筑解析系统 V296](./static-build-resolver-v296.md)
