...
32. `V32` source-entry summary alignment：当前阶段已冻结到 source-entry summary alignment；目标是去掉 `resolve-build-source-entries` 对 `collection.summary` 的最后一处高层 aliasing，让高层 tool 与底层 `ResolveStaticBuildSourceEntriesResult.summary` 直接使用同一组 key，见 [静态构筑解析系统 V32](./static-build-resolver-v32.md)
