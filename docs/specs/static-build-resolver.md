...
35. `V35` matrix row compact contract：当前阶段已冻结到 row-level compact contract；目标是为 `ResolveStaticBuildSkillMatrixRow` 增加稳定的 `resolvedBuckets / assumptions / unsupportedEffects`，让 `resolve-build-skill-matrix` 不再从 `row.build` 手工抽取这些紧凑字段，见 [静态构筑解析系统 V35](./static-build-resolver-v35.md)
36. `V35` matrix row compact contract：当前阶段已收口；`ResolveStaticBuildSkillMatrixRow` 已新增稳定的 `resolvedBuckets / assumptions / unsupportedEffects`，`resolve-build-skill-matrix` 不再从 `row.build` 手工抽取这些紧凑字段，见 [静态构筑解析系统 V35](./static-build-resolver-v35.md)
