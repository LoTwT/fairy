...
36. `V36` matrix row explanation contract：当前阶段已冻结到 row-level explanation contract；目标是为 `ResolveStaticBuildSkillMatrixRow` 增加稳定的 `diagnostics / sourceNotes`，让 `resolve-build-skill-matrix` 不再依赖完整 `row.build` 才能输出结构化行级解释，见 [静态构筑解析系统 V36](./static-build-resolver-v36.md)
