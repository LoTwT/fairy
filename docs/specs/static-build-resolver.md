...
313. `V313` compact matrix and utility scalar contracts：当前阶段进行中；当前目标是为 `compact.ts` 中 `skill-matrix` 与 `source-utility-view` 剩余最大的匿名 scalar/list contract 补显式 compact alias，不改变任何运行时行为
