...
47. `V47` skill-matrix summary aggregates：当前阶段已冻结范围；目标是为 `ResolveStaticBuildSkillMatrixResult` 增加聚合 `diagnosticSummary / sourceNoteSummary`，使 skill matrix 与 trigger matrix / source views / source-entry collection 在顶层摘要能力上保持对称，见 [静态构筑解析系统 V47](./static-build-resolver-v47.md)
