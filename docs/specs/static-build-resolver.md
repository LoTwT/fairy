...
19. `V19` legacy utility engine closeout：当前已完成 `V19.1` scope freeze 与 `V19.2` inventory；最后两个仍未收口的 utility-only 旧通用音擎 `「月相」-朔 / 「电磁暴」-叁式` 将按 source note 收口，而不新增能量相关 public key，见 [静态构筑解析系统 V19](./static-build-resolver-v19.md)
