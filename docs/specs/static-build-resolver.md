...
48. `V48` skill-matrix row summaries：当前阶段：已完成 scope freeze；目标是在不改变 `rows[].diagnostics / sourceNotes` 原始数组的前提下，为 `StaticBuildSkillMatrixRow` 新增稳定 `diagnosticSummary / sourceNoteSummary`，让行级 contract 与 `source-view entry / trigger row` 对齐，见 [静态构筑解析系统 V48](./static-build-resolver-v48.md)
