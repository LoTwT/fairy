...
41. `V41` diagnostic summaries：当前阶段已冻结范围；下一步只为 `source-damage-view entry` 与 `trigger-entry matrix row` 补稳定的 `diagnosticSummary`，不改现有 `diagnostics[]`，见 [静态构筑解析系统 V41](./static-build-resolver-v41.md)
