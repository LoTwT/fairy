...
26. `V26` unified source-entry collection summary：当前已完成 scope freeze；目标是在不扩新 coverage、不新增 snapshot key 的前提下，为 `resolveStaticBuildSourceEntries()` 增加稳定 summary、分组与排序 contract，见 [静态构筑解析系统 V26](./static-build-resolver-v26.md)
