...
28. `V28` source-view summary contracts：当前已完成 scope freeze；目标是在不扩新 coverage、不新增 snapshot key 的前提下，为 `resolveStaticBuildSourceDamageViews()` 与 `resolveStaticBuildSourceUtilityViews()` 增加稳定 `summary`，并固定 standalone / delta / trigger / rate 的分组与排序语义，见 [静态构筑解析系统 V28](./static-build-resolver-v28.md)
