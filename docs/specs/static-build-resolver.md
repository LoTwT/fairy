...
283. `V283` build diagnostic key list contracts：当前阶段进行中；当前目标是为 diagnostic 公开 contract 中的 `keys` 补显式公开列表 type，并统一复用，不改变任何运行时行为，见 [静态构筑解析系统 V283](./static-build-resolver-v283.md)
