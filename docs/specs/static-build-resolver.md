...
27. `V27` trigger-entry matrix summary：当前已完成 scope freeze；目标是在不扩新 coverage、不新增 snapshot key 的前提下，为 `resolveStaticBuildTriggerMatrix()` 增加稳定 `summary`，并固定 `main-formula / source-view` 的分组与排序语义，见 [静态构筑解析系统 V27](./static-build-resolver-v27.md)
