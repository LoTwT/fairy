...
8. `V8` assumption ownership 已进入 contract freeze，目标是把 anomaly / disorder 剩余 assumptions 统一归到 `finalPanel` / `dynamicSnapshot` / `stateSnapshot` / `resolvedSnapshot` / 真动态过程，而不是继续无边界扩 snapshot contract，见 [静态构筑解析系统 V8](./static-build-resolver-v8.md)
