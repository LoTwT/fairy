...
33. `V33` support-scope normalization：当前阶段已冻结到 support-scope normalization；目标是把 `resolve-build-*` 高层 tool 的 unsupported / support-scope 组装与 catalog helper 统一到 `resolve-build-shared.ts`，见 [静态构筑解析系统 V33](./static-build-resolver-v33.md)
