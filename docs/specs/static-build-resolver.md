...
279. `V279` build unsupported-effect list contracts：当前阶段进行中；当前目标是为 single-build / skill-matrix 的 `unsupportedEffects` 补显式公开列表 type，并统一复用，不改变任何运行时行为，见 [静态构筑解析系统 V279](./static-build-resolver-v279.md)
