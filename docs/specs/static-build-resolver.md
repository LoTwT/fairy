...
51. `V51` trigger-matrix row resolve summaries：当前阶段：已完成 scope freeze；目标是在不改变 `damage / requirementSummary / diagnosticSummary / sourceNoteSummary` 的前提下，为 `StaticBuildTriggerMatrixRow` 新增稳定 `summary`，让 trigger row 直接暴露 `ResolveStaticBuildResult.summary`，见 [静态构筑解析系统 V51](./static-build-resolver-v51.md)
