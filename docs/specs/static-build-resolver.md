...
11. `V11` structured source notes：已完成 `V11.2`，`ResolveStaticBuildResult` 已新增结构化 `sourceNotes`，下一步让 `zzz-agent` 与 source view 优先消费结构化 note，同时保持旧 assumptions 兼容，见 [静态构筑解析系统 V11](./static-build-resolver-v11.md)
