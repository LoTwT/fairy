...
278. `V278` build assumption list contracts：当前阶段进行中；当前目标是为 single-build / source views / source-entry / trigger-matrix / skill-matrix 的 `assumptions` 补显式公开列表 type，并统一复用，不改变任何运行时行为，见 [静态构筑解析系统 V278](./static-build-resolver-v278.md)
