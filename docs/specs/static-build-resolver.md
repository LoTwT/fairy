...
10. `V10` source-specific delta view：下一阶段继续扩 `source-specific damage view`，但只处理当前 contract 下能通过“含快照结果 - 去除快照结果”稳定表达的 delta source；第一批固定为 `爱芮 [异放]`，`霰落星殿` 与 `混沌重金属 4件` 先保留为 research-only，见 [静态构筑解析系统 V10](./static-build-resolver-v10.md)
