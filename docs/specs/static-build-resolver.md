...
408. `V408` buhflipexplode enemy record contracts：当前阶段已收口；`buhflipexplode/index.ts` 中顶层 enemy record 已统一复用显式 alias，不改变任何 raw/published JSON shape、enemy field contract 或 enemy ref list 语义，见 [静态构筑解析系统 V408](./static-build-resolver-v408.md)
409. `V409` buhflipexplode multiplier table contracts：当前阶段已收口；`buhflipexplode/index.ts` 中四个 multiplier table 常量已统一复用显式 readonly list alias，不改变任何 multiplier 数值、公式 helper 或 node level 常量语义，见 [静态构筑解析系统 V409](./static-build-resolver-v409.md)
410. `V410` buhflipexplode node level list contracts：当前阶段已收口；`buhflipexplode/index.ts` 中 `SD/TS` node level 常量列表已统一复用显式 readonly list alias，不改变任何 node level 数值、boundary 常量或公式 helper 语义，见 [静态构筑解析系统 V410](./static-build-resolver-v410.md)
