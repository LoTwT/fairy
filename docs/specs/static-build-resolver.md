...
17. `V17` generic drive-disc curated coverage：当前已完成 `V17.2` inventory；`拂晓生花`、`流光咏叹`、`獠牙重金属`、`如影相随`、`折枝剑歌` 已冻结为 Batch A，`沧浪行歌`、`囚徒手记` 已冻结为 Batch B，显式排除了 teamwide / off-carrier / 非 damage 主 contract 的通用驱动盘，见 [静态构筑解析系统 V17](./static-build-resolver-v17.md)
