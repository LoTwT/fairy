...
39. `V39` trigger row source metadata：当前阶段已冻结范围；下一步只为 `trigger-entry matrix row` 补稳定来源追溯字段，不改 trigger formula、不扩 coverage，见 [静态构筑解析系统 V39](./static-build-resolver-v39.md)
