...
46. `V46` trigger-matrix summary aggregates：当前阶段已冻结范围；目标是为 `resolveStaticBuildTriggerMatrix().summary` 增加聚合 `diagnosticSummary / sourceNoteSummary`，使 trigger matrix 与 source views / source-entry collection 在顶层摘要能力上保持对称，见 [静态构筑解析系统 V46](./static-build-resolver-v46.md)
