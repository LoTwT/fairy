...
49. `V49` skill-matrix row resolve summaries：当前阶段：已完成 scope freeze；目标是在不改变 `damageSummary / resolvedBuckets / diagnosticSummary / sourceNoteSummary` 的前提下，为 `StaticBuildSkillMatrixRow` 新增稳定 `summary`，让行级 contract 直接暴露 `ResolveStaticBuildResult.summary`，见 [静态构筑解析系统 V49](./static-build-resolver-v49.md)
