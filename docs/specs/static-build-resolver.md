...
319. `V319` loadout selection contracts：当前阶段进行中；当前目标是把 `build/types.ts` 与 `definitions.ts` 中仍然宽泛使用 `StaticBuildCatalogId` 或 inline object 的 loadout 选择输入统一收成显式公开 contract，不改变任何运行时行为
