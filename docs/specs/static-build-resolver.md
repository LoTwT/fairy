...
40. `V40` requirement summaries：当前阶段已冻结范围；下一步只为 `source-damage-view entry` 与 `trigger-entry matrix row` 补稳定的 `requirementSummary`，不改现有 `requirements[]`，见 [静态构筑解析系统 V40](./static-build-resolver-v40.md)
