...
18. `V18` legacy attack signature closeout：当前已完成 `V18.1` scope freeze 与 `V18.2` inventory；最后一批 legacy 强攻签名固定为 `可琳 / 比利 / 安东 / 家政员 / 仿制星徽引擎 / 旋钻机-赤轴`，其中姿态、距离、持续命中与真动态额外结算继续保持 source note，不新增新的 public key，见 [静态构筑解析系统 V18](./static-build-resolver-v18.md)
