...
34. `V34` matrix row damage summary：当前阶段已冻结到 matrix row damage-summary contract；目标是为 `ResolveStaticBuildSkillMatrixRow` 增加稳定 `damageSummary`，让 `resolve-build-skill-matrix` 不再手工拼装 `row.damage`，见 [静态构筑解析系统 V34](./static-build-resolver-v34.md)
