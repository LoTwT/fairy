...
12. `V12` structured diagnostics：已完成 `V12.3 coverage / unsupported diagnostics`，当前 `ResolveStaticBuildResult.diagnostics` 已覆盖 `defaulted-input`、`coverage-gap`、`unsupported-effect` 三类结构化镜像；下一步进入 `V12.4 consumer adoption`，见 [静态构筑解析系统 V12](./static-build-resolver-v12.md)
