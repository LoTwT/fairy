...
37. `V37` compact helper exports：当前阶段已冻结到 compact helper exports；目标是把 `zzz-agent` 高层的 `compactMatrix / compactTriggerMatrix / compactSourceEntries` 下沉为 `zzz-data` 可复用导出，见 [静态构筑解析系统 V37](./static-build-resolver-v37.md)
