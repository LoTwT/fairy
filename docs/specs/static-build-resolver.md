...
16. `V16` generic w-engine curated coverage：已完成 `V16.3` Batch B；当前 `鎏金花信`、`星徽引擎`、`「月相」-晦`、`「月相」-望`、`青漪灵鼎`、`电波漫步`、`「电磁暴」-壹式`、`「电磁暴」-贰式` 已补为 curated coverage，`「灰烬」-钴蓝` 已收口为 source-aware unsupported，`加农转子` 与 `幻变魔方` 已按 partial coverage 纳入稳定 bucket，下一步只剩 Batch C 评估，见 [静态构筑解析系统 V16](./static-build-resolver-v16.md)
