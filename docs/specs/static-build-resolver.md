...
14. `V14` non-agent source-specific damage views：已完成 scope freeze；下一步评估 `霰落星殿`、`混沌重金属 4件` 是否应从 `research-only` 提升为独立 source view，同时继续明确 `轰鸣座驾`、`自由蓝调 4件` 不纳入此阶段，见 [静态构筑解析系统 V14](./static-build-resolver-v14.md)
