...
7. `V7` resolved snapshot overrides 已完成 `V7.3` 前三批来源迁移，当前已把 `柏妮思 M6` 的 `25% 火抗无视` 接到 `scenario.resolvedSnapshot.bucketDeltas.ignoreResistance`，并把 `格莉丝 M2`、`简`、`派派`、`时流贤者` 的异常倍率折算收口到 `scenario.resolvedSnapshot.multiplierFactors.skillMultiplierFactor`，见 [静态构筑解析系统 V7](./static-build-resolver-v7.md)
