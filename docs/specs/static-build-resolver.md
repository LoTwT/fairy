...
17. `V17` generic drive-disc curated coverage：当前已完成 `V17.3`；`拂晓生花`、`流光咏叹`、`獠牙重金属`、`如影相随`、`折枝剑歌`、`沧浪行歌`、`囚徒手记` 已补为 curated / partial coverage，显式排除了 teamwide / off-carrier / 非 damage 主 contract 的通用驱动盘，下一步进入 `V17.4` closeout，见 [静态构筑解析系统 V17](./static-build-resolver-v17.md)
