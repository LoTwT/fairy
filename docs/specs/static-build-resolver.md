...
16. `V16` generic w-engine curated coverage：已完成 `V16.3` Batch C；当前 `鎏金花信`、`星徽引擎`、`「月相」-晦`、`「月相」-望`、`青漪灵鼎`、`电波漫步`、`「电磁暴」-壹式`、`「电磁暴」-贰式` 已补为 curated coverage，`「灰烬」-钴蓝` 已收口为 source-aware unsupported，`加农转子`、`幻变魔方`、`强音热望`、`街头巨星` 已纳入稳定 bucket / partial coverage，下一步进入 `V16.4 closeout`，见 [静态构筑解析系统 V16](./static-build-resolver-v16.md)
