...
280. `V280` build combat-tag list contracts：当前阶段进行中；当前目标是为 build-layer 公开 contract 中的 `combatTags` 补显式公开列表 type，并统一复用，不改变任何运行时行为，见 [静态构筑解析系统 V280](./static-build-resolver-v280.md)
