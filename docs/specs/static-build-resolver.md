...
187. `V187` explicit compact skill-matrix row metadata：当前阶段已收口；compact `skill-matrix row.metadata` 已改为显式 compact type，不再直接复用 raw row metadata type，从而把 skill-matrix row compact contract 与 `V186` 继续对齐，见 [静态构筑解析系统 V187](./static-build-resolver-v187.md)
188. `V188` explicit compact trigger row metadata：当前阶段已收口；compact `trigger-matrix row.metadata` 已改为显式 compact type，不再直接复用 raw row metadata type，从而把 trigger-row compact contract 与 `V187` 继续对齐，见 [静态构筑解析系统 V188](./static-build-resolver-v188.md)
189. `V189` explicit compact entry damage summaries：当前阶段已收口；compact `trigger-row / source-damage-view entry` 的 `damage` 已改为显式 compact type，不再直接复用 raw damage summary shape，从而把 entry 级 compact damage contract 与 `V188` 继续对齐，见 [静态构筑解析系统 V189](./static-build-resolver-v189.md)
190. `V190` explicit compact nested build details：当前阶段已收口；compact `skill-matrix / trigger-matrix / source-damage-view` 在 `includeDetails` 下的 nested `build` 已改为显式 compact build，不再直接复用 raw `ResolveStaticBuildResult`，从而把 compact includeDetails contract 与 `V189` 继续对齐，见 [静态构筑解析系统 V190](./static-build-resolver-v190.md)
191. `V191` explicit compact source utility view contracts：当前阶段已收口；compact `source-utility-view` 的 entry/meta/summary 已改为显式 compact types，不再通过 indexed access 复用 raw utility entry contract，见 [静态构筑解析系统 V191](./static-build-resolver-v191.md)
...
204. `V204` explicit compact helper input contracts：当前阶段已收口；`compact.ts` 的导出 helper 参数签名已统一为显式公开类型，不再通过 indexed access 复用上游字段；同时 `calculator/types.ts` 已补 `DamageBreakdown`、`build/types.ts` 已补 `StaticBuildEntryDamage`，见 [静态构筑解析系统 V204](./static-build-resolver-v204.md)
205. `V205` build-tool unsupported scope alignment：当前阶段已收口；`zzz-agent` 高层 `source-damage-view / source-utility-view / source-entry collection` tool 已统一 unsupported / coverage-gap message 的 scope label，并保持 `source-entry collection` 的自有 `supportedSourceViewAgents / supportedUtilityWEngines` key 不漂移，见 [静态构筑解析系统 V205](./static-build-resolver-v205.md)
206. `V206` source-entry coverage-gap helper contracts：当前阶段已收口；`source-entry collection` 最后一批匿名 coverage-gap 返回已收进 shared helper，utility-only 与 mixed/anomaly 路径继续保留 `supportedUtilityWEngines / supportedSourceViewAgents` 这些 tool 自有字段名，见 [静态构筑解析系统 V206](./static-build-resolver-v206.md)
207. `V207` build-tool success response contracts：当前阶段已收口；6 个高层 build tool 的成功返回已收进 shared helper，并继续保留 `build / matrix / views / collection` 这些 tool 自有字段名，见 [静态构筑解析系统 V207](./static-build-resolver-v207.md)
208. `V208` source-entry tool typed panel normalization：当前阶段已收口；`resolve-build-source-entries.ts` 已去掉 `panel / scenario` 两处 `as any`，并继续保留 utility-only partial `finalPanel` 输入能力，见 [静态构筑解析系统 V208](./static-build-resolver-v208.md)
...
222. `V222` source-utility coverage response helper contracts：当前阶段已收口；`resolve-build-source-utility-views.ts` 的 missing / uncovered 分支已收进 shared helper，不改变任何 tool 的输入输出 shape，见 [静态构筑解析系统 V222](./static-build-resolver-v222.md)
223. `V223` source-damage coverage response helper contracts：当前阶段已收口；`resolve-build-source-damage-views.ts` 的 uncovered 分支已收进 shared helper，不改变任何 tool 的输入输出 shape，见 [静态构筑解析系统 V223](./static-build-resolver-v223.md)
224. `V224` source-entry utility-only loadout support helper contracts：当前阶段已收口；`resolve-build-source-entries.ts` 的 `utilityOnly` catalog / w-engine support 选择已收进 shared helper，不改变任何 tool 的输入输出 shape，见 [静态构筑解析系统 V224](./static-build-resolver-v224.md)
...
229. `V229` source-entry collection response helper contracts：当前阶段已收口；`resolve-build-source-entries.ts` 的空结果 coverage / 非空 success 后处理已收进 shared helper，不改变任何 tool 的输入输出 shape，见 [静态构筑解析系统 V229](./static-build-resolver-v229.md)
230. `V230` source-utility response helper contracts：当前阶段已收口；`resolve-build-source-utility-views.ts` 的 missing / uncovered / success 后处理已收进 shared helper，不改变任何 tool 的输入输出 shape，见 [静态构筑解析系统 V230](./static-build-resolver-v230.md)
...
246. `V246` build-tool scope key contracts：当前阶段已收口；高层 build tool 已固定显式 `BuildToolScopeKey` / `BuildToolScopeLabel` type，并让 `buildToolScopeLabels` 仅作为满足这些 contract 的值对象存在，不改变任何 tool 的输入输出 shape，见 [静态构筑解析系统 V246](./static-build-resolver-v246.md)
247. `V247` build-tool damage attribute contracts：当前阶段已收口；高层 build tool 已固定显式 `NormalizedAttributeKey` / `NormalizedSpecialtyKey` / `BaseDamageAttribute` type，并让属性映射表仅作为满足这些 contract 的值对象存在，不改变任何 tool 的输入输出 shape，见 [静态构筑解析系统 V247](./static-build-resolver-v247.md)
248. `V248` build-tool scenario input contracts：当前阶段已收口；高层 build tool 已固定显式 `BuildToolScenarioInput` / `BuildToolSkillMatrixContextInput` 及其公共输入 type，并让 schema 仅负责校验，不改变任何 tool 的输入输出 shape，见 [静态构筑解析系统 V248](./static-build-resolver-v248.md)
249. `V249` build-tool finalPanel interface contracts：当前阶段已收口；高层 build tool 已固定显式 `BuildToolFinalPanelInput` interface，并让 `finalPanelSchema` 仅负责校验，不改变任何 tool 的输入输出 shape，见 [静态构筑解析系统 V249](./static-build-resolver-v249.md)
250. `V250` build-tool specialty label contracts：当前阶段已收口；高层 build tool 已固定显式 `BuildToolSpecialtyKey` / `BuildToolSpecialtyLabel` type，并让 `specialtyLabels` 仅作为满足这些 contract 的值对象存在，不改变任何 tool 的输入输出 shape，见 [静态构筑解析系统 V250](./static-build-resolver-v250.md)
251. `V251` build-tool catalog specialty contracts：当前阶段已收口；高层 build tool 已固定可参数化的 `CatalogItem<TSpecialty>` 与共享 `SpecialtyCatalogItem` alias，并让 loadout / execution / response helper 统一复用它们，不改变任何 tool 的输入输出 shape，见 [静态构筑解析系统 V251](./static-build-resolver-v251.md)
252. `V252` build drive-disc set input contracts：当前阶段已收口；build-layer 已固定显式 `StaticBuildDriveDiscSetsInput` type，并让 source-damage-view / source-utility-view helper 统一复用它，不改变任何公开函数的输入输出 shape，见 [静态构筑解析系统 V252](./static-build-resolver-v252.md)
253. `V253` build-tool source-entry context contracts：当前阶段已收口；高层 build tool 已让 source-entry context / execution helper 统一改用显式 `StaticBuildScenarioInput` / `StaticBuildFinalPanelInput` type，不改变任何 tool 的输入输出 shape，见 [静态构筑解析系统 V253](./static-build-resolver-v253.md)
254. `V254` build snapshot helper contracts：当前阶段已收口；build-layer 已让 `definitions.ts` 与 `resolver.ts` 统一复用显式 `StaticBuildDynamicSnapshotInput` / `StaticBuildStateSnapshotInput` / `StaticBuildDynamicValueKey` / `StaticBuildDynamicCountKey` / `StaticBuildStateValueKey`，不改变任何运行时行为，见 [静态构筑解析系统 V254](./static-build-resolver-v254.md)
255. `V255` build source-note source type contracts：当前阶段已收口；`definitions.ts` 的 source-note / coverage helper 已统一复用显式 `StaticBuildSourceType`，不改变任何运行时行为，见 [静态构筑解析系统 V255](./static-build-resolver-v255.md)
256. `V256` build source-note condition key contracts：当前阶段已收口；`StaticBuildSourceNote` 已统一复用显式 `StaticBuildDynamicFlagKey` / `StaticBuildDynamicCountKey` / `StaticBuildDynamicValueKey` / `StaticBuildStateFlagKey` / `StaticBuildStateValueKey` / `StaticBuildDamageType` / `AnomalyType`，不改变任何运行时行为，见 [静态构筑解析系统 V256](./static-build-resolver-v256.md)
257. `V257` build progression scalar contracts：当前阶段已收口；build-layer 已新增显式 `StaticBuildDriveDiscPieces` / `StaticBuildAgentLevel` / `StaticBuildAgentMindscape` / `StaticBuildCoreSkillLevel` / `StaticBuildWEngineRefinement`，并让 `utility-views.ts` 与 source-note helper 统一复用这些标量 type，不改变任何运行时行为，见 [静态构筑解析系统 V257](./static-build-resolver-v257.md)
258. `V258` build-tool progression scalar contracts：当前阶段已收口；高层 build tool 的 loadout helper 已统一复用显式 `StaticBuildDriveDiscPieces` / `StaticBuildAgentLevel` / `StaticBuildAgentMindscape` / `StaticBuildCoreSkillLevel` / `StaticBuildWEngineRefinement`，并把这些 scalar type 从 `zzz-data` 正式导出，不改变任何 tool 的输入输出 shape，见 [静态构筑解析系统 V258](./static-build-resolver-v258.md)
259. `V259` build-tool preset catalog entry contracts：当前阶段已收口；高层 build-tool preset 已统一复用显式 `StaticBuildAgentCatalogEntry` / `StaticBuildUtilityAgentCatalogEntry`，不再通过 `(typeof supportedStaticBuild...)[number]` 推导 item type，不改变任何 tool 的输入输出 shape，见 [静态构筑解析系统 V259](./static-build-resolver-v259.md)
260. `V260` build-tool schema trait contracts：当前阶段已收口；高层 build-tool schema 已统一复用显式 `StaticBuildSkillTag` / `StaticBuildDamageType` / `StaticBuildDriveDiscPieces`，并把驱动盘 schema 收进共享 `driveDiscSetSchema`，不改变任何 tool 的输入输出 shape，见 [静态构筑解析系统 V260](./static-build-resolver-v260.md)
261. `V261` build effect trait contracts：当前阶段已收口；`StaticBuildEffectCondition.minimumMindscape` 与 `StaticBuildEffectDefinition.sourceType` 已统一复用显式 `StaticBuildAgentMindscape` / `StaticBuildSourceType`，不改变任何运行时行为，见 [静态构筑解析系统 V261](./static-build-resolver-v261.md)
262. `V262` build condition threshold iteration contracts：当前阶段已收口；`resolver.ts` 已用显式 typed threshold helper 统一处理 `minimumDynamicCounts / minimumDynamicValues / minimumStateValues`，不再依赖 `keyof NonNullable<...>` cast，不改变任何运行时行为，见 [静态构筑解析系统 V262](./static-build-resolver-v262.md)
263. `V263` build scalar snapshot contracts：当前阶段已收口；build-layer 与高层 schema 已统一复用显式 `StaticBuildEnergyGenerationRate` / `StaticBuildAnomalyMastery` / `StaticBuildResolvedAnomalyProficiency` / `StaticBuildRemainingTime`，不改变任何运行时行为，见 [静态构筑解析系统 V263](./static-build-resolver-v263.md)
264. `V264` build final panel scalar contracts：当前阶段已收口；build-layer 与高层 schema 已统一复用显式 `StaticBuildAttack` / `StaticBuildBaseAttack` / `StaticBuildCritRate` / `StaticBuildCritDamage` / `StaticBuildHP` / `StaticBuildSheerForce` / `StaticBuildAnomalyProficiency` / `StaticBuildAnomalyCritRate` / `StaticBuildAnomalyCritDamage` / `StaticBuildPenetrationRate` / `StaticBuildPenetrationValue`，不改变任何运行时行为，见 [静态构筑解析系统 V264](./static-build-resolver-v264.md)
265. `V265` build enemy scalar contracts：当前阶段已收口；build-layer 与高层 schema 已统一复用显式 `StaticBuildDefenderBaseDefense` / `StaticBuildDefenderResistance` / `StaticBuildDefenseBonus` / `StaticBuildDefenseReduction` / `StaticBuildResistanceReduction` / `StaticBuildIgnoreResistance` / `StaticBuildVulnerabilityBonus` / `StaticBuildDamageReduction` / `StaticBuildStunVulnerability` / `StaticBuildNonStunVulnerability` / `StaticBuildSpecialMultiplier`，并让 `attackerLevel` 统一复用 `StaticBuildAgentLevel`，不改变任何运行时行为，见 [静态构筑解析系统 V265](./static-build-resolver-v265.md)
266. `V266` build scenario multiplier input contracts：当前阶段已收口；build-layer 与高层 schema 已统一复用显式 `StaticBuildSkillMultiplierInputValue` / `StaticBuildDamageMultiplierInputValue`，不改变任何运行时行为，见 [静态构筑解析系统 V266](./static-build-resolver-v266.md)
267. `V267` build effect stack and resolved threshold contracts：当前阶段已收口；build-layer 已统一复用显式 `StaticBuildEffectStacks`，并让 resolved-threshold 条件统一复用 `StaticBuildCritRate` / `StaticBuildResolvedAnomalyProficiency`，不改变任何运行时行为，见 [静态构筑解析系统 V267](./static-build-resolver-v267.md)
268. `V268` build resolved bucket scalar contracts：当前阶段已收口；build-layer 已统一复用显式 `StaticBuildAttackPercent` / `StaticBuildFlatAttack` / `StaticBuildBonusDamageSum` / `StaticBuildSheerBonusSum` / `StaticBuildAnomalyBonusDamageSum` / `StaticBuildSkillMultiplierFactor`，并让 `StaticBuildResolvedBuckets` 与既有的 `crit / penetration / defense / resistance / vulnerability / anomaly` 标量 contract 保持一致，不改变任何运行时行为，见 [静态构筑解析系统 V268](./static-build-resolver-v268.md)
269. `V269` build damage result scalar contracts：当前阶段已收口；build-layer 已统一复用显式 `StaticBuildBaseDamageValue` / `StaticBuildExpectedTotal` / `StaticBuildCriticalTotal` / `StaticBuildNonCriticalTotal` / `StaticBuildModifierValue`，并让 `resolvedPanel / resolveSummary / entryDamage / rowDamageSummary / traceModifier` 与既有的 scalar contract 保持一致，不改变任何运行时行为，见 [静态构筑解析系统 V269](./static-build-resolver-v269.md)
270. `V270` build summary core count contracts：当前阶段已收口；build-layer 已统一复用显式 `StaticBuildDiagnosticCount` / `StaticBuildSourceNoteCount` / `StaticBuildAssumptionCount` / `StaticBuildUnsupportedEffectCount`，并让 `diagnostic / source-note / assumption / unsupported-effect` 相关 summary 与既有 scalar contract 保持一致，不改变任何运行时行为，见 [静态构筑解析系统 V270](./static-build-resolver-v270.md)
271. `V271` build requirement count contracts：当前阶段已收口；build-layer 已统一复用显式 `StaticBuildRequirementCount` / `StaticBuildSatisfiedRequirementCount` / `StaticBuildUnsatisfiedRequirementCount`，并让 requirement summary / utility entry summary 与既有 scalar contract 保持一致，不改变任何运行时行为，见 [静态构筑解析系统 V271](./static-build-resolver-v271.md)
272. `V272` build view and matrix count contracts：当前阶段已收口；build-layer 已统一复用显式 `StaticBuildGroupCount` / `StaticBuildSupportedCount` / `StaticBuildUnsupportedCount` / `StaticBuildEntryCount` / `StaticBuildStandaloneCount` / `StaticBuildDeltaCount` / `StaticBuildTriggerCount` / `StaticBuildRateCount` / `StaticBuildSourceDamageViewCount` / `StaticBuildSourceUtilityViewCount` / `StaticBuildRowCount` / `StaticBuildMainFormulaCount` / `StaticBuildSourceViewCount`，并让 `source-damage-view / source-utility-view / source-entry / trigger-matrix / skill-matrix` 的 summary / group summary 与 `StaticBuildEntryCaveatSummary` 保持一致，不改变任何运行时行为，见 [静态构筑解析系统 V272](./static-build-resolver-v272.md)
273. `V273` build effect-summary count contracts：当前阶段已收口；build-layer 已统一复用显式 `StaticBuildAppliedEntryCount` / `StaticBuildTotalEntryCount` / `StaticBuildAppliedRowCount` / `StaticBuildTotalRowCount`，并让 `source-damage-view / trigger-matrix / skill-matrix` 的 effect-summary 与既有 count contract 保持一致，不改变任何运行时行为，见 [静态构筑解析系统 V273](./static-build-resolver-v273.md)
274. `V274` build source-utility value contracts：当前阶段已收口；build-layer 已统一复用显式 `StaticBuildUtilityValue` / `StaticBuildCooldownSeconds`，并让 `source-utility-view` 的 utility entry / entry summary 与既有 scalar contract 保持一致，不改变任何运行时行为，见 [静态构筑解析系统 V274](./static-build-resolver-v274.md)
275. `V275` build skill-matrix summary scalar contracts：当前阶段已收口；build-layer 已让 `skill-matrix summary` 的 `baseDamageValue / attack / hp / sheerForce / critRate / critDamage / penetrationRate / penetrationValue` 统一复用既有显式 scalar type，不改变任何运行时行为，见 [静态构筑解析系统 V275](./static-build-resolver-v275.md)
276. `V276` build summary map contracts：当前阶段已收口；build-layer 已新增显式 `StaticBuildBucketValueMap` / `StaticBuildFormulaMultiplierMap`，并让 `StaticBuildResolveSummary.formulaMultipliers` 与 `skill-matrix summary/group summary` 的 `commonBuckets / commonFormulaMultipliers` 统一复用这些 type，不改变任何运行时行为，见 [静态构筑解析系统 V276](./static-build-resolver-v276.md)
277. `V277` build summary variable list contracts：当前阶段已收口；build-layer 已新增显式 `StaticBuildVariableBucketList` / `StaticBuildVariableFormulaMultiplierList`，并让 `skill-matrix summary/group summary` 的 `variableBuckets / variableFormulaMultipliers` 统一复用这些 type，不改变任何运行时行为，见 [静态构筑解析系统 V277](./static-build-resolver-v277.md)
...
285. `V285` build message text contracts：当前阶段已收口；build-layer 已新增显式 `StaticBuildSourceNoteMessage` / `StaticBuildDiagnosticMessage`，并让 source-note / diagnostic 公开 contract 中的 `message` 统一复用这些 type，不改变任何运行时行为，见 [静态构筑解析系统 V285](./static-build-resolver-v285.md)
286. `V286` build effect id contracts：当前阶段已收口；build-layer 已新增显式 `StaticBuildEffectId`，并让 effect override / definition / trace / effect summary 中的 `effectId` 统一复用这个 type，不改变任何运行时行为，见 [静态构筑解析系统 V286](./static-build-resolver-v286.md)
287. `V287` build source-name text contracts：当前阶段已收口；build-layer 已新增显式 `StaticBuildSourceName`，并让 effect definition / trace / effect summaries 中的 `sourceName` 统一复用这个 type，不改变任何运行时行为，见 [静态构筑解析系统 V287](./static-build-resolver-v287.md)
288. `V288` build effect label text contracts：当前阶段已收口；build-layer 已新增显式 `StaticBuildEffectLabel`，并让 effect definition / trace / effect summaries 中的 `label` 统一复用这个 type，不改变任何运行时行为，见 [静态构筑解析系统 V288](./static-build-resolver-v288.md)
...
295. `V295` build catalog id contracts：当前阶段已收口；loadout / catalog 相关公开 contract 中的通用 catalog `id` 已统一复用显式公开 type，不改变任何运行时行为，见 [静态构筑解析系统 V295](./static-build-resolver-v295.md)
296. `V296` build entry and row id contracts：当前阶段已收口；source-note / source-view / matrix row 相关公开 contract 中的通用 `id` 已统一复用显式公开 type，不改变任何运行时行为，见 [静态构筑解析系统 V296](./static-build-resolver-v296.md)
...
312. `V312` compact source and trigger text contracts：当前阶段已收口；`trigger-matrix / source-damage-views / source-utility-views / source-entry collection` 的 compact entry/meta/result contract 已统一复用显式 `id / label / sourceId / canonicalLabel / stableKey / assumptions`，不改变任何运行时行为，见 [静态构筑解析系统 V312](./static-build-resolver-v312.md)
313. `V313` compact matrix and utility scalar contracts：当前阶段已收口；`compact.ts` 中 `skill-matrix` 与 `source-utility-view` 剩余最大的匿名 scalar/list contract 已统一复用显式 compact alias，不改变任何运行时行为，见 [静态构筑解析系统 V313](./static-build-resolver-v313.md)
...
334. `V334` assumption and unsupported-effect set contracts：当前阶段已收口；`skill-matrix / trigger-matrix` 中 assumptions 与 unsupported-effects 去重已统一收成显式公开 contract，不改变任何运行时行为，见 [静态构筑解析系统 V334](./static-build-resolver-v334.md)
335. `V335` assumption and combat-tag set reuse：当前阶段已收口；`source-entry` 的 assumptions 去重集合与 `resolver / skill-matrix` 的 combat-tag 去重集合已统一复用既有显式公开 set contract，不改变任何运行时行为，见 [静态构筑解析系统 V335](./static-build-resolver-v335.md)
336. `V336` catalog source item contracts：当前阶段已收口；`catalog.ts` 中读取上游 `agents.json / w-engines.json` 的 source item shape 与 helper 参数已统一复用显式公开 contract，不改变任何运行时行为，见 [静态构筑解析系统 V336](./static-build-resolver-v336.md)
337. `V337` skill-matrix template contracts：当前阶段已收口；`matrix.ts` 中 skill-matrix template / source-stat helper 的本地 interface 与裸参数已统一复用显式公开 contract，不改变任何运行时行为，见 [静态构筑解析系统 V337](./static-build-resolver-v337.md)
338. `V338` source-view requirement key helper contracts：当前阶段已收口；`views.ts / utility-views.ts` 中 requirement helper 的 `key` 参数已统一复用既有 `StaticBuildRequirementKey`，不改变任何运行时行为，见 [静态构筑解析系统 V338](./static-build-resolver-v338.md)
...
351. `V351` agent utility helper text contracts：当前阶段已收口；`zzz-agent` utility helper 文本 contract 已统一复用显式公开 alias，不改变任何匹配、归一化或文本清洗逻辑，见 [静态构筑解析系统 V351](./static-build-resolver-v351.md)
...
388. `V388` gachabase bangboo and drive-disc raw field contracts：当前阶段已收口；`gachabase/types.ts` 里 bangboo 与 drive-disc 相关 raw published field 已统一复用显式 alias / named interface，不改变任何公开 JSON shape或 helper 逻辑，见 [静态构筑解析系统 V388](./static-build-resolver-v388.md)
389. `V389` game-modes wave object contracts：当前阶段已收口；`game-modes.ts` 里剩余的 wave object 已统一复用显式 named interface，不改变任何 published JSON shape、`cleaned` helper 语义或上层 resolver 逻辑，见 [静态构筑解析系统 V389](./static-build-resolver-v389.md)
390. `V390` gachabase agent skin asset contracts：当前阶段已收口；`gachabase/types.ts` 中 `AgentSkin.assets` 已统一复用显式 named interface，不改变任何 published JSON shape 或上层 helper 逻辑，见 [静态构筑解析系统 V390](./static-build-resolver-v390.md)
391. `V391` game-modes buff record key contracts：当前阶段已收口；`game-modes.ts` 中 `BuffsJson` 已统一复用显式 record-key alias，不改变任何 published JSON shape、buff field contract 或上层 helper 逻辑，见 [静态构筑解析系统 V391](./static-build-resolver-v391.md)
392. `V392` buhflipexplode wave object contracts：当前阶段已收口；`buhflipexplode/index.ts` 中 `SDSide.waves` 与 `TSSide.waves` 已统一复用显式 named interface，不改变任何 raw/published JSON shape、公式 helper 或上层 resolver 逻辑，见 [静态构筑解析系统 V392](./static-build-resolver-v392.md)
...
394. `V394` buhflipexplode version record key contracts：当前阶段已收口；`buhflipexplode/index.ts` 中 `SD/DA/TS` 的版本 record key 已统一复用显式 alias，见 [静态构筑解析系统 V394](./static-build-resolver-v394.md)
395. `V395` buhflipexplode enemy db key contracts：当前阶段已收口；`buhflipexplode/index.ts` 中 `BuhflipEnemyDB` 的顶层 record key 已统一复用显式 enemy-id alias，见 [静态构筑解析系统 V395](./static-build-resolver-v395.md)
396. `V396` buhflipexplode buff text contracts：当前阶段已收口；`buhflipexplode/index.ts` 中 `buffName / buffDesc / buffNames` 已统一复用显式 buff 文本 alias，见 [静态构筑解析系统 V396](./static-build-resolver-v396.md)
397. `V411` buhflipexplode milestone status sync：当前阶段已收口；`V394~V396` 在总规格/索引/架构中的历史状态说明已统一同步，不改变任何运行时代码或公开 contract，见 [静态构筑解析系统 V411](./static-build-resolver-v411.md)
398. `V397` buhflipexplode element multiplier tuple contracts：当前阶段已收口；`buhflipexplode/index.ts` 中敌人与 SD side 的元素倍率五元组已统一复用显式 tuple alias，见 [静态构筑解析系统 V397](./static-build-resolver-v397.md)
399. `V398` buhflipexplode text pair contracts：当前阶段已收口；`buhflipexplode/index.ts` 中敌人 `desc / perf` 的匿名文本二元 tuple 已统一复用显式 alias，见 [静态构筑解析系统 V398](./static-build-resolver-v398.md)
400. `V399` buhflipexplode enemy ref id contracts：当前阶段已收口；`buhflipexplode/index.ts` 中 `SD/DA/TS` 敌人引用的 `id` 已统一复用 `BuhflipEnemyId`，见 [静态构筑解析系统 V399](./static-build-resolver-v399.md)
401. `V400` buhflipexplode version text contracts：当前阶段已收口；`buhflipexplode/index.ts` 中 `SD/DA/TS` 版本容器的 `versionName / versionTime / mode name` 已统一复用显式 alias，见 [静态构筑解析系统 V400](./static-build-resolver-v400.md)
402. `V401` buhflipexplode enemy text contracts：当前阶段已收口；`buhflipexplode/index.ts` 中敌人 `name / image / misc / spoiler*` 文本字段已统一复用显式 alias，见 [静态构筑解析系统 V401](./static-build-resolver-v401.md)
403. `V402` buhflipexplode enemy ref list contracts：当前阶段已收口；`buhflipexplode/index.ts` 中 `SD/DA/TS` 敌人引用集合已统一复用显式 list alias，见 [静态构筑解析系统 V402](./static-build-resolver-v402.md)
404. `V403` buhflipexplode wave list contracts：当前阶段已收口；`buhflipexplode/index.ts` 中 `SD/TS` wave 集合已统一复用显式 list alias，见 [静态构筑解析系统 V403](./static-build-resolver-v403.md)
405. `V404` buhflipexplode side slot list contracts：当前阶段已收口；`buhflipexplode/index.ts` 中 `SD/TS` side slot 集合已统一复用显式 alias，见 [静态构筑解析系统 V404](./static-build-resolver-v404.md)
406. `V405` buhflipexplode node list contracts：当前阶段已收口；`buhflipexplode/index.ts` 中 `SD/TS` node 集合已统一复用显式 list alias，见 [静态构筑解析系统 V405](./static-build-resolver-v405.md)
407. `V406` buhflipexplode version record contracts：当前阶段已收口；`buhflipexplode/index.ts` 中 `SD/DA/TS` version record 容器已统一复用显式 alias，不改变任何 raw/published JSON shape、公式 helper 或 node/mode list 语义，见 [静态构筑解析系统 V406](./static-build-resolver-v406.md)
408. `V407` buhflipexplode versions mode list contracts：当前阶段已收口；`buhflipexplode/index.ts` 中 `SD/TS` 顶层 mode list 已统一复用显式 list alias，不改变任何 raw/published JSON shape、公式 helper 或 version record/data 语义，见 [静态构筑解析系统 V407](./static-build-resolver-v407.md)
409. `V408` buhflipexplode enemy record contracts：当前阶段已收口；`buhflipexplode/index.ts` 中顶层 enemy record 已统一复用显式 alias，不改变任何 raw/published JSON shape、enemy field contract 或 enemy ref list 语义，见 [静态构筑解析系统 V408](./static-build-resolver-v408.md)
410. `V409` buhflipexplode multiplier table contracts：当前阶段已收口；`buhflipexplode/index.ts` 中四个 multiplier table 常量已统一复用显式 readonly list alias，不改变任何 multiplier 数值、公式 helper 或 node level 常量语义，见 [静态构筑解析系统 V409](./static-build-resolver-v409.md)
411. `V410` buhflipexplode node level list contracts：当前阶段已收口；`buhflipexplode/index.ts` 中 `SD/TS` node level 常量列表已统一复用显式 readonly list alias，不改变任何 node level 数值、boundary 常量或公式 helper 语义，见 [静态构筑解析系统 V410](./static-build-resolver-v410.md)
412. `V412` game-modes deadly-assault list contracts：当前阶段已收口；`game-modes.ts` 中 `Deadly Assault` 的 `DABuff[] / DAEnemyItem[] / DAVersionItem[]` 已统一复用显式 list alias，不改变任何 published JSON shape、字段语义或上层 helper 逻辑，见 [静态构筑解析系统 V412](./static-build-resolver-v412.md)
413. `V413` game-modes shiyu-defense enemy and wave list contracts：当前阶段已收口；`game-modes.ts` 中 `Shiyu Defense` 的 `SDEnemyItem[] / SDWaveItem[]` 已统一复用显式 list alias，不改变任何 published JSON shape、字段语义或上层 helper 逻辑，见 [静态构筑解析系统 V413](./static-build-resolver-v413.md)
414. `V414` game-modes shiyu-defense side and node list contracts：当前阶段已收口；`game-modes.ts` 中 `Shiyu Defense` 的 `SDSideItem | null` slot list 与 `SDNodeItem[]` 已统一复用显式 alias，不改变任何 published JSON shape、字段语义或上层 helper 逻辑，见 [静态构筑解析系统 V414](./static-build-resolver-v414.md)
415. `V415` game-modes shiyu-defense version and mode list contracts：当前阶段已收口；`game-modes.ts` 中 `Shiyu Defense` 的 `SDVersionItem[] / SDModeItem[]` 已统一复用显式 list alias，不改变任何 published JSON shape、字段语义或上层 helper 逻辑，见 [静态构筑解析系统 V415](./static-build-resolver-v415.md)
416. `V416` game-modes threshold-simulation enemy and wave list contracts：当前阶段已收口；`game-modes.ts` 中 `Threshold Simulation` 的 boss/regular `enemy[] / wave[]` 已统一复用显式 list alias，不改变任何 published JSON shape、字段语义或上层 helper 逻辑，见 [静态构筑解析系统 V416](./static-build-resolver-v416.md)
417. `V417` game-modes threshold-simulation side and node list contracts：当前阶段已收口；`game-modes.ts` 中 `Threshold Simulation` 的 `TSBossSideItem | TSRegularSideItem | null` slot list 与 `TSNodeItem[]` 已统一复用显式 alias，不改变任何 published JSON shape、字段语义或上层 helper 逻辑，见 [静态构筑解析系统 V417](./static-build-resolver-v417.md)
418. `V418` game-modes threshold-simulation version and mode list contracts：当前阶段已收口；`game-modes.ts` 中 `Threshold Simulation` 的 `TSVersionItem[] / TSModeItem[]` 已统一复用显式 list alias，不改变任何 published JSON shape、字段语义或上层 helper 逻辑，见 [静态构筑解析系统 V418](./static-build-resolver-v418.md)
419. `V419` cleaned encounter and DA buff text list contracts：当前阶段已收口；`cleaned/types.ts` 中候选名、弱点、抗性与 `DA buff` 名字列表已统一复用显式 alias，不改变任何 `cleaned` helper 语义或 build resolver 行为，见 [静态构筑解析系统 V419](./static-build-resolver-v419.md)
420. `V420` cleaned shiyu-defense view list contracts：当前阶段已收口；`cleaned/types.ts` 中 `Shiyu Defense` 视图的敌人列表、buff 名称/描述列表与 side 列表已统一复用显式 alias，不改变任何 `cleaned` helper 语义或 build resolver 行为，见 [静态构筑解析系统 V420](./static-build-resolver-v420.md)
421. `V421` cleaned threshold-simulation view list contracts：当前阶段已收口；`cleaned/types.ts` 中 `Threshold Simulation` 视图的 boss/regular 敌人列表、buff 名称列表与 side 列表已统一复用显式 alias，不改变任何 `cleaned` helper 语义或 build resolver 行为，见 [静态构筑解析系统 V421](./static-build-resolver-v421.md)
422. `V422` cleaned encounter helper list contracts：当前阶段已收口；`cleaned encounter` helper 与返回类型中 `matches`、候选名、弱点与抗性列表已统一复用显式 alias，不改变任何匹配逻辑、`cleaned` helper 语义或 build resolver 行为，见 [静态构筑解析系统 V422](./static-build-resolver-v422.md)
423. `V423` gachabase agent list label contracts：当前阶段已收口；`gachabase/types.ts` 中 `AgentListItem.attributes` 与 `AgentListItem.attackTypes` 已统一复用显式 alias，不改变任何 published JSON shape、lookup 行为或 build resolver 逻辑，见 [静态构筑解析系统 V423](./static-build-resolver-v423.md)
424. `V424` gachabase shared skill list contracts：当前阶段已收口；`gachabase/types.ts` 中共享的 `StatBoost`、`AgentSkillDescription` 与 `AgentSkillStat` 列表已统一复用显式 alias，并同步复用到 `AgentPromotion`、`AgentSkillGroup`、`CoreSkillLevel` 与 `BangbooOptimization`，不改变任何 published JSON shape、lookup 行为或 build resolver 逻辑，见 [静态构筑解析系统 V424](./static-build-resolver-v424.md)
425. `V425` gachabase agent details outer list contracts：当前阶段已收口；`gachabase/types.ts` 中 `agent-details.json` 外层资源与内容列表已统一复用显式 alias，不改变任何 published JSON shape、lookup 行为或 build resolver 逻辑，见 [静态构筑解析系统 V425](./static-build-resolver-v425.md)
426. `V426` gachabase remaining outer list contracts：当前阶段已收口；`gachabase/types.ts` 中 `w-engine`、`bangboo` 与 `drive-disc` 剩余外层列表已统一复用显式 alias，不改变任何 published JSON shape、lookup 行为或 build resolver 逻辑，见 [静态构筑解析系统 V426](./static-build-resolver-v426.md)
427. `V427` lookup-bangboo trimmed-result contracts：当前阶段已收口；`lookup-bangboo` 的顶层 trimmed-result key 已统一复用显式 alias，不改变任何邦布查询、筛选或属性计算逻辑，见 [静态构筑解析系统 V427](./static-build-resolver-v427.md)
...
470. `V470` cleaned encounter and DA buff text contracts：当前阶段已收口；`cleaned/types.ts` 中 encounter / DA buff 的文本字段已统一复用 `game-modes` 上游 alias，不改变任何 `cleaned` helper 返回内容、文案值或视图逻辑，见 [静态构筑解析系统 V470](./static-build-resolver-v470.md)
471. `V471` cleaned enemy damage context scalar contracts：当前阶段已收口；`cleaned/types.ts` 中 `EnemyDamageContext` 的核心标识与数值字段已统一复用 `game-modes` 上游 alias，不改变任何 `cleaned` helper 计算逻辑、上下文字段语义或 encounter 选择逻辑，见 [静态构筑解析系统 V471](./static-build-resolver-v471.md)
472. `V472` cleaned encounter view scalar contracts：当前阶段已收口；`cleaned/types.ts` 中 encounter/view 的位置索引、数量与 side 标量字段已统一复用显式 alias 或 `game-modes` 上游数值 alias，不改变任何 `cleaned` helper 的遍历顺序、筛选逻辑或返回值，见 [静态构筑解析系统 V472](./static-build-resolver-v472.md)
473. `V473` cleaned encounter and node-buff text contracts：当前阶段已收口；`cleaned/types.ts` 中 encounter 候选名与 `SD/TS node buff` 文本字段已统一复用 `game-modes` 上游 alias，不改变任何 `cleaned` helper 的筛选逻辑、返回值或列表结构，见 [静态构筑解析系统 V473](./static-build-resolver-v473.md)
474. `V474` resolve-build dynamic snapshot object contracts：当前阶段已收口；`resolve-build-schemas.ts` 中 `dynamicSnapshot` 的匿名嵌套对象已统一复用显式 interface，不改变任何 Zod schema 校验规则、序列化结构或 resolver/tool 行为，见 [静态构筑解析系统 V474](./static-build-resolver-v474.md)
475. `V475` resolve-build state snapshot object contracts：当前阶段已收口；`resolve-build-schemas.ts` 中 `stateSnapshot` 的匿名嵌套对象已统一复用显式 interface，不改变任何 Zod schema 校验规则、序列化结构或 resolver/tool 行为，见 [静态构筑解析系统 V475](./static-build-resolver-v475.md)
476. `V476` resolve-build resolved snapshot object contracts：当前阶段已收口；`resolve-build-schemas.ts` 中 `resolvedSnapshot` 的匿名嵌套对象已统一复用显式 interface，不改变任何 Zod schema 校验规则、序列化结构或 resolver/tool 行为，见 [静态构筑解析系统 V476](./static-build-resolver-v476.md)
477. `V477` resolve-build effect override contracts：当前阶段已收口；`resolve-build-schemas.ts` 中 `effectOverrides[]` 的匿名对象已统一复用显式 interface 与共享 schema，不改变任何字段集合、校验规则、序列化结构或 resolver/tool 行为，见 [静态构筑解析系统 V477](./static-build-resolver-v477.md)
478. `V478` resolve-build dynamic snapshot nested schemas：当前阶段已收口；`resolve-build-schemas.ts` 中 `dynamicSnapshot` 的匿名 nested Zod schema 已统一复用共享 schema 常量，不改变任何字段集合、校验规则、序列化结构或 resolver/tool 行为，见 [静态构筑解析系统 V478](./static-build-resolver-v478.md)
479. `V479` resolve-build state snapshot nested schemas：当前阶段已收口；`resolve-build-schemas.ts` 中 `stateSnapshot` 的匿名 nested Zod schema 已统一复用共享 schema 常量，不改变任何字段集合、校验规则、序列化结构或 resolver/tool 行为，见 [静态构筑解析系统 V479](./static-build-resolver-v479.md)
480. `V480` resolve-build resolved snapshot nested schemas：当前阶段已收口；`resolve-build-schemas.ts` 中 `resolvedSnapshot` 的匿名 nested Zod schema 已统一复用共享 schema 常量，不改变任何字段集合、校验规则、序列化结构或 resolver/tool 行为，见 [静态构筑解析系统 V480](./static-build-resolver-v480.md)
...
485. `V485` resolve-build scenario scalar schemas：当前阶段已收口；`resolve-build-schemas.ts` 中 `scenario/context` 的重复 scalar schema 已统一复用共享 schema 常量，不改变字段值域、默认值、校验规则、分支结构、序列化结构或 resolver/tool 行为，见 [静态构筑解析系统 V485](./static-build-resolver-v485.md)
486. `V486` resolve-build dynamic snapshot scalar schemas：当前阶段已收口；`resolve-build-schemas.ts` 中 `dynamicSnapshot` 的 leaf scalar schema 已统一复用共享 schema 常量，不改变字段值域、默认值、校验规则、对象结构、序列化结构或 resolver/tool 行为，见 [静态构筑解析系统 V486](./static-build-resolver-v486.md)
487. `V487` resolve-build state snapshot scalar schemas：当前阶段已收口；`resolve-build-schemas.ts` 中 `stateSnapshot` 的 leaf scalar schema 已统一复用共享 schema 常量，不改变字段值域、默认值、校验规则、对象结构、序列化结构或 resolver/tool 行为，见 [静态构筑解析系统 V487](./static-build-resolver-v487.md)
488. `V488` resolve-build resolved snapshot scalar schemas：当前阶段已收口；`resolve-build-schemas.ts` 中 `resolvedSnapshot` 的 delta/factor leaf scalar schema 已统一复用共享 schema 常量，不改变字段值域、默认值、校验规则、对象结构、序列化结构或 resolver/tool 行为，见 [静态构筑解析系统 V488](./static-build-resolver-v488.md)
489. `V489` resolve-build drive-disc set scalar schemas：当前阶段已收口；`resolve-build-schemas.ts` 中 `driveDiscSet` 的 leaf schema 已统一复用共享 schema 常量，不改变字段值域、默认值、校验规则、对象结构、序列化结构或 resolver/tool 行为，见 [静态构筑解析系统 V489](./static-build-resolver-v489.md)
490. `V490` resolve-build effect override scalar schemas：当前阶段已收口；`resolve-build-schemas.ts` 中 `effectOverride` 的 leaf schema 已统一复用共享 schema 常量，不改变字段值域、默认值、校验规则、对象结构、序列化结构或 resolver/tool 行为，见 [静态构筑解析系统 V490](./static-build-resolver-v490.md)
491. `V491` resolve-build combat tag element schema：当前阶段已收口；`resolve-build-schemas.ts` 中 `combatTags` 的元素 schema 已统一复用共享 schema 常量，不改变字段值域、默认值、校验规则、对象结构、序列化结构或 resolver/tool 行为，见 [静态构筑解析系统 V491](./static-build-resolver-v491.md)
492. `V492` resolve-build scenario damage-type literal schemas：当前阶段已收口；`resolve-build-schemas.ts` 中 `scenario.damageType` 的 branch literal schema 已统一复用共享 schema 常量，不改变字段值域、默认值、校验规则、分支结构、序列化结构或 resolver/tool 行为，见 [静态构筑解析系统 V492](./static-build-resolver-v492.md)
493. `V493` resolve-build scenario multiplier scalar schemas：当前阶段已收口；`resolve-build-schemas.ts` 中 `scenarioSkillMultiplier / scenarioDamageMultiplier` 共享的 `number|string` scalar schema 已统一复用共享 schema 常量，不改变字段值域、默认值、校验规则、union 结构、序列化结构或 resolver/tool 行为，见 [静态构筑解析系统 V493](./static-build-resolver-v493.md)
