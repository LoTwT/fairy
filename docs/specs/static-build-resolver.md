...
29. `V29` main resolver summary contract：当前阶段已冻结到单次 resolver summary；目标是为 `ResolveStaticBuildResult` 增加稳定 `summary`，收口单场景结果的公式乘区摘要与 diagnostics / sourceNotes / unsupportedEffects 的分组统计，见 [静态构筑解析系统 V29](./static-build-resolver-v29.md)
