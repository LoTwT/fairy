...
289. `V289` build group label text contracts：当前阶段进行中；当前目标是为各类 group summary 公开 contract 中的 `label` 补显式公开 text type，并统一复用，不改变任何运行时行为，见 [静态构筑解析系统 V289](./static-build-resolver-v289.md)
