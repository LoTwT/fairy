...
287. `V287` build source-name text contracts：当前阶段进行中；当前目标是为 effect 公开 contract 中的 `sourceName` 补显式公开 text type，并统一复用，不改变任何运行时行为，见 [静态构筑解析系统 V287](./static-build-resolver-v287.md)
