...
276. `V276` build summary map contracts：当前阶段进行中；当前目标是为 `StaticBuildResolveSummary.formulaMultipliers` 与 `skill-matrix summary/group summary` 的 `commonBuckets / commonFormulaMultipliers` 补显式公开 map type，并统一复用，不改变任何运行时行为，见 [静态构筑解析系统 V276](./static-build-resolver-v276.md)
