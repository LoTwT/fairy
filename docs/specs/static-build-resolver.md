...
291. `V291` build skill-matrix group key contracts：当前阶段进行中；当前目标是为 `StaticBuildSkillMatrixRow.group` 补显式公开 key type，并统一复用，不改变任何运行时行为，见 [静态构筑解析系统 V291](./static-build-resolver-v291.md)
