...
50. `V50` source-damage-view entry resolve summaries：当前阶段：已完成 scope freeze；目标是在不改变 `damage / requirementSummary / diagnosticSummary / sourceNoteSummary` 的前提下，为 `StaticBuildSourceDamageViewEntry` 新增稳定 `summary`，让 source-damage-view entry 直接暴露 `ResolveStaticBuildResult.summary`，见 [静态构筑解析系统 V50](./static-build-resolver-v50.md)
