...
25. `V25` second-batch utility / resource views：当前已完成 scope freeze；目标是在不扩大主伤害公式的前提下，为 `时光切片` 这类 `喧响值 + 能量` 资源条目扩展 utility / source-entry contract，并把 utility-only 查询从 damage-agent catalog 解耦，见 [静态构筑解析系统 V25](./static-build-resolver-v25.md)
