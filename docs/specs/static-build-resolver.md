...
31. `V31` core skill matrix effect summary：当前阶段已冻结到矩阵 applied effect summary contract；目标是为 `ResolveStaticBuildSkillMatrixResult` 增加稳定 `effectSummary`，把当前只存在于高层 tool 的增益清单聚合逻辑下沉到 `zzz-data` public contract，见 [静态构筑解析系统 V31](./static-build-resolver-v31.md)
