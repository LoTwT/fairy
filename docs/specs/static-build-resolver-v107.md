...
但 mixed collection 这条统一入口里，`entry.diagnosticSummary` 虽然底层从更早阶段起就稳定暴露，当前仍缺：

1. source-entry collection 断言没有把 `entry.diagnosticSummary` 当成 mixed entry 的正式消费层
2. Agent prompt 还没明确要求在 mixed collection 路径优先读取 `entry.diagnosticSummary`
3. README 也还没把这层写成 source-entry collection 的公共 contract

`V107` 只解决一件事：

- 在 unified `source-entry collection` 路径，把 mixed entry 的 `diagnosticSummary` 对齐成正式公共 contract
...
1. `entry.diagnosticSummary.count`
2. `entry.diagnosticSummary.hasDiagnostics`
3. `entry.diagnosticSummary.hasDefaultedInput`
4. `entry.diagnosticSummary.hasCoverageGap`
...
1. 让 unified `source-entry collection` 的测试显式断言 mixed entry 的 `diagnosticSummary`
2. 更新 Agent prompt，要求在 mixed collection 路径优先读取 `entry.diagnosticSummary`
...
1. unified `source-entry collection` 测试显式校验 mixed entry 的 `diagnosticSummary`
2. Agent prompt 明确要求 mixed collection 路径优先读取 `entry.diagnosticSummary`
