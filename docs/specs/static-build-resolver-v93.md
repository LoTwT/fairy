...
`V93` 只处理 `source-damage-view groups[*]` 的稳定 `caveatSummary`，不扩到单条 `entry`，也不同时改 `source-utility-view`、`source-entry collection` 或 `trigger-matrix`。

## 目标

1. 为 `StaticBuildSourceDamageViewGroupSummary` 新增稳定 `caveatSummary`
2. 保持 group-level caveat 与现有：
   - `groups[*].assumptionSummary`
   - `groups[*].supportedCount / unsupportedCount`
     的统计一致
3. 让上层按组拆 standalone / delta 条目时，不再手工组合组级 caveat
...
1. 不为单条 `source-damage-view entry` 新增 `caveatSummary`
