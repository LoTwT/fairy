...
当前常规伤害公式只支持调用方已经确认本次计算不涉及未实现特殊机制的场景。特殊机制边界由
[特殊乘区规范](../factors/special.md)统一维护：

- 已确认不适用特殊机制时，可以正常调用本公式；
- 已知适用但尚未实现，或无法确认是否适用时，不得调用本公式并声称结果完整；
- 本公式不接收任意预计算的特殊倍率，也不为特殊乘区保留占位字段。
