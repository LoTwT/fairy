...
export interface StunDamageFactorInput {
  ...
  readonly stunDamageMultiplierAdjustments: readonly number[]
}
...
- `isStunned` 记录受击方当前是否失衡。
- `baseStunDamageMultiplier` 是当前状态下已经确定的基础乘数。
- `stunDamageMultiplierAdjustments` 保存本次计算适用的失衡易伤倍率调整。

`baseStunDamageMultiplier` 是可以直接参与乘法的倍率，不是攻略公式中需要再加 `1` 的比例贡献。
例如受击方在当前状态下的基础失衡易伤贡献为 `50%` 时，调用方应把它换算为 `1.5` 后传入，而不是
传入 `0.5`。`stunDamageMultiplierAdjustments` 的每个成员是对该倍率的有符号加算调整；游戏文本中的
...
> 待确认：攻略中的“未失衡时失衡易伤倍率”在本地 Nanoka 3.0 中尚未找到稳定的独立英文术语。本
> 规范暂不为它建立新的公开英文名称；该分支只通过 `isStunned: false` 表达，最终 review 时人工确认。

## 数组语义

- `stunDamageMultiplierAdjustments` 为空数组时，调整总和为 `0`。
...
```text
未钳制值 = baseStunDamageMultiplier + Σ stunDamageMultiplierAdjustments

失衡易伤区结果 = clamp(未钳制值, 0.2, 5)，isStunned 为 true 时
失衡易伤区结果 = clamp(未钳制值, 1, 3)，isStunned 为 false 时
```

钳制在全部调整求和后执行。必须先检查未钳制值是否有限，结果不执行取整或截断。

`baseStunDamageMultiplier` 已经包含基础值，因此公式不会再额外加 `1`。同一项动态调整不能既被
调用方预先计入 `baseStunDamageMultiplier`，又作为数组成员传入。

## 输入值域

- `baseStunDamageMultiplier` 必须是非负有限数。
- `stunDamageMultiplierAdjustments` 的成员允许任意有限有符号数值；正数表示提升，负数表示降低。
