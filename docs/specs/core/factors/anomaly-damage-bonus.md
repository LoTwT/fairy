...
异常伤害公式同时采用[已结算增伤区](settled-damage-bonus.md)、减易伤区和异常增伤区，但同一项数值
贡献只能进入符合其语义的一个乘区：

- 普通异常和普通紊乱路径中，通用的“造成的伤害提升或降低”先在每次有效异常积蓄发生时由
  [增伤区](damage-bonus.md)结算，再由[虚拟代理人快照](../helpers/virtual-agent-snapshot.md)对最终倍率
  加权；其他路径只有在自身可靠规则已经明确最终倍率时，才能建立已结算增伤区输入；
