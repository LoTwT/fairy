...
`DisorderDazeDealtFactorInput` 直接表示
[虚拟代理人快照](../helpers/virtual-agent-snapshot.md)中的 `dazeDealtFactorResult`。输入是已经完成记录
筛选、溢出裁剪和加权的最终倍率，不是百分比贡献。加权结果 `1.2` 直接以 `1.2` 传入，不能转换为
`dazeDealtIncreases: [0.2]` 并声称该数组保存真实效果贡献。

本乘区不接收异常积蓄记录、权重、代理人身份或效果来源，也不重复快照帮助函数的加权规则。
...
如果快照帮助函数因没有有效代理人记录而失败，调用方不得以默认输入代替缺失快照并声称结果完整。
