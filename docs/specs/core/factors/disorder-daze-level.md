...
积蓄记录加权和向下取整，由[虚拟代理人快照帮助函数](../helpers/virtual-agent-snapshot.md)的 `level` 字段
提供。因此不增加只有同名字段的对象包装，也不在乘区内再次取整。
...
调用方应将原异常状态保存的虚拟代理人快照 `level` 直接传入。记录筛选、溢出裁剪、加权、最终
向下取整和无有效记录时的失败行为由快照规范统一维护。

乘区只接收最终整数等级，不接收异常积蓄记录、权重、原始等级数组或完整快照。
...
如果快照帮助函数因没有有效代理人记录而失败，调用方不得使用等级 `1` 替代缺失输入并声称结果完整。
