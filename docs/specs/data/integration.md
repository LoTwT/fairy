...
**状态：规则 v4；单代理人纯整合、离线全量新制品构建、确定性字节与摘要、总索引及完整复验、显式 pnpm 整合与验证命令已实现。固定当前数据集的增量写入、事务恢复与互斥读取已实现；公开 API 尚未实现。**
...
- 首次创建要求目标不存在。已存在的空目标目录、无标记制品、符号链接、外来文件或损坏状态均明确拒绝，不自动接管或递归重建。
- 标记存在时，必须匹配规范化后的目标路径与协议版本；每次操作核对控制目录的精确允许集合。
- update、recover 与持锁读取共用锁入口。在打开 SQLite 数据库之前，先按上述允许集合检查控制目录成员名，
...
- 空控制目录可以初始化；标记写入前中断，只允许在目标不存在、控制目录至多含锁文件与初始 `state.next` 时重新初始化；其他残留报错。
...
复用全量构建器和验证器；[命令入口](../../../packages/data/scripts/current-nanoka-agents.ts)提供以下独立命令，
不改变 `integrate:nanoka:agents` 位置参数的意义。从仓库根目录执行：
...
更新回执含 `outcome`（`committed` 或 `unchanged`）、目标与维护报告绝对路径、成员/语言/输入/输出计数，
