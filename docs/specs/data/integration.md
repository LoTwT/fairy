...
**状态：规则 v4；单代理人纯整合、离线全量新制品构建、确定性字节与摘要、总索引及完整复验、统一 pnpm 生成与独立验证命令已实现。固定当前数据集的增量写入、事务恢复与互斥读取已实现；公开读取与 npm 导出见[消费契约](consumption.md)。多实体 v3 根索引与多类别完整制品的内部生成、验证及 v2 → v3 转换已实现（第 7.1、9 节），正常生成/恢复/验证命令、公开读取 API、类型生成与发布副本尚未切换。**
...
本节与第 9 节的单实体实现描述当前已发布、由正常入口使用的 v2 外壳；多实体 v3 外壳的内部契约见第 7.1 节。
两级外壳都遵守同一条原则：`index.json` 只描述完成并验证的产物，不保存重试、进度或待写入状态。
...
### 7.1 多实体根索引 v3（内部能力，尚未切换正常入口）

多实体外壳把单一代理人索引改为按已接入类别分块：根只描述整个快照的来源身份、来源版本与已包含类别，
类别块分别描述成员范围、完整性、整合规则、详情语言、来源输入与输出文件摘要。生成、验证与 v2 → v3 转换
已实现（第 9 节），正常生成/恢复/验证命令、公开读取 API、类型生成与静态发布副本仍在 v2 外壳上运行。
...
该边界只约束转换输入，不改变既有恢复对历史规则、历史语言配置与历史预算配置的支持范围。
...
- 生成入口取得同一永久锁后，只有 `state.json` 不存在且控制目录至多含锁文件、journal 与初始 `state.next` 时，才允许初始化已有制品。
  复用完整验证器检查当前格式、支持的当前规则 v4、当前完整语言配置、成员、来源清单、精确文件集合及实际字节摘要；
  以已验证索引的实际字节摘要、规则版本和验证配置建立 `idle(current)`，再进入既有恢复及候选更新流程。
...
旧规则和旧语言配置按状态记录的验证配置检查文件外壳与完整性（历史语言允许已支持列表的非空、无重复子集）；新候选始终由当前代码规则及当前来源配置重建，
不把改版本标记视作规则迁移。除显式生成入口按上述初始化规则登记合法已有制品外，未登记或损坏旧目录明确拒绝，由用户保留现场处理，不自动修复或丢弃。
...
候选复验通过后记录 `prepared(before | null, after)`，其中描述符包含索引字节摘要、规则版本与验证所用来源配置。
状态路径全部由控制目录固定推导，不执行记录中的任意清理路径。回滚还原旧目标后先记录 `idle(before)`，再删除候选，避免清理中断后误判为新集已安装。
...
内部 `withNanokaCurrentDataset` 回调用于落实这个协议，不增加 npm 查询 API；回调返回前必须读完所需字节，
不能只返回路径后释放锁再读。遇到活跃操作报 `BUSY`；遇到 `prepared` 或未清理工作材料报 `RECOVERY_REQUIRED`，
明确拒绝并要求先恢复再重试。维护命令 `verify:nanoka:current` 使用此契约。
原独立制品验证器与 `verify:nanoka:agents` 继续面向静态副本，不持当前锁，不可替代当前目录并发读取协议。
...
全量构建器作为内部能力由统一生成入口调用，不再提供单独的临时生成 package script；用户无需分步构建、初始化、更新。
生成命令与回执见[当前数据更新实现与命令](#当前数据更新实现与命令)，使用说明见[data README](../../../packages/data/README.md#生成-integrated)。
静态制品可通过 `pnpm --filter @randomplay/data verify:nanoka:agents /absolute/copy/integrated` 独立复验。
独立验证接受一个非空目录参数；帮助、错误及回执遵守相同 CLI 契约。
验证回执含实际绝对 `artifactDirectory`、`agentCount`、`detailLocales` 与 `verified: true`。
...
[索引类型](../../../packages/data/src/integration/snapshot-types.ts)为包内类型，本版不从根入口导出。
...
本能力只用于内部生成、验证与转换，测试使用合成 fixture 与临时目录；正常生成、恢复与验证命令、当前目录事务与
管理记录协议、公开读取 API、类型生成与静态发布副本尚未切换，真实 raw 与 integrated 未做迁移。

### 当前数据更新实现与命令

[当前数据事务模块](../../../packages/data/scripts/nanoka-integration/current.ts) 实现第 8.1 节协议，
复用全量构建器和验证器；[命令入口](../../../packages/data/scripts/current-nanoka-agents.ts)提供唯一生成入口及独立维护命令。
`generate:integrated` 不绑定实体名称，目前仅处理已实现的代理人整合，后续实体接入同一入口；本次不扩展实体。
从仓库根目录执行：
...
生成回执含 `outcome`（`committed` 或 `unchanged`）、目标与维护报告绝对路径、成员/语言/输入/输出计数，
另含 `reusedEntityFiles`、`changedEntityFiles`、`removedEntityFiles` 以及未知字段和 codeName 差异数量。
`changedEntityFiles` 统计本次候选中新建或字节改变的实体文件，首次生成时包含全部实体文件；
`reusedEntityFiles` 统计字节相同并复用旧 inode 的实体文件，`removedEntityFiles` 统计旧集中不再存在的实体文件。
三者均不含总索引，也不是临时构建的写入次数。
恢复回执含目标路径、`outcome`（`committed`、`rolled-back` 或 `unchanged`）及 `available`；验证回执沿用已有验证字段。
恢复的 `unchanged` 指数据集未变，也可能创建初始标记或清理上次残留工作目录；`available` 表明是否已有可用数据集。
`BUSY` 要求重试，`RECOVERY_REQUIRED` 要求先恢复，`INVALID_STATE` 保留异常现场；`UPDATE_FAILED` 保留
原错误及恢复结果。即便命令最终因报告、清理或输出失败退出 1，也不能据此推断提交未发生，应恢复并复验。

[当前数据合成测试](../../../packages/data/test/agent-current.test.ts)使用人工合成输入覆盖更新、旧规则/语言迁移、
非法输入保持旧集、实际字节变化、完整文件集合与摘要、受管理目录归属、同进程及跨进程互斥、读取租约、
报告/rename/硬链接/部分清理失败，并比较实际 inode、设备号、纳秒 mtime 和摘要，监听写文件调用以证明当前文件未重写。
...
增量更新和恢复流程复用本契约、当前纯函数与全量构建器。下列是完整流程的验收清单；单实体与
全量新制品、跳过重写、成员删除、现有目录替换及中断恢复均已纳入合成测试，真实 raw 不作为 CI 前提：
...
并接入显式 pnpm 整合与验证命令。步骤三已实现固定当前数据集的更新协议、增量识别与跳过重写、完整索引驱动的成员移除、规则升级、现有数据集的
整组替换、互斥读取与中断恢复。运行时查询 API、制品发布渠道和人工计算模型分别在其对应任务中确定。
