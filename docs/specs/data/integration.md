...
**状态：规则 v4；包内单代理人纯整合函数、正式类型与离线合成测试已实现。全量文件生成、增量写入及公开 API 尚未实现。**
...
生产写入器必须在实现时提供整组写入、失败恢复和并发读取约束，并通过中断实验验收；本轮未实现该协议。
...
本步仅处理已解析 JSON，验证来源值、必需字段、可选结构、共享一致性、保真与对象结果确定性。
文件读取、严格 UTF-8 解码、原始字节摘要、序列化、全量索引、CLI、增量写入和故障恢复均不在本步，
不以这里的对象测试声称这些文件流程已经验证。未实现计算 helpers、definitions、core 映射或公开查询 API。
...
该轮生成的是用于验证当时契约的本地完整资料；当时尚未实现生产整合工具。当前新增的纯函数仍不包含
整合命令、增量写入、故障恢复、运行时查询 API 或发布；本地一次性验证脚本不能当作生产工具。此前安比 v2 的实体文件与仅改文件名的数据副本保持原状，不能冒充 v4 产物。

## 10. 后续实现的验收入口

后续全量生成和写入流程应复用本契约与当前纯函数。下列是完整流程的验收清单；单实体部分已通过合成测试，
涉及输入集合、字节或写入的项目仍待实现，真实 raw 不作为 CI 前提：
...
7. 普通 build、test、pack 保持离线，不读取真实缓存，不触发抓取或整合写入；测试只对合成输入调用纯函数，输出与维护材料按制品边界分别检查。
8. `codeName` 的等值、不同拼写、空字符串、缺失和错误类型；按配置顺序选值，读取完成顺序变化不影响结果，
   后续语言不覆盖，且相同豁免不适用于其他共享字段。

本步已将正式类型、单实体整合和合成验收纳入 data 包常规检查。后续需实现完整输入集合读取、严格解码、
摘要和序列化、全量索引生成，再补足增量写入协议、篡改拒绝与故障恢复验收，最后接入工作区命令；
运行时查询 API、制品发布渠道和人工计算模型分别在其对应实现任务中确定。本地一次性构建器不能直接当作生产工具。
