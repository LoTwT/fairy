...
出现未完成的旧协议事务时先按旧协议恢复到稳定状态，只有显式迁移才写入新协议，不覆盖旧日志强行升级。
...
`migrate:nanoka:current` 是唯一把 v2 数据集变成 v3 外壳的操作，区分五种输入：
...
v2 静态/受管理/旧协议 prepared 到 v3 的三条迁移路径、迁移中断与重复迁移、历史规则/语言/预算恢复，
