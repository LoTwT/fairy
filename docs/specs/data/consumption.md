...
42 个邦布、293 个怪物、59 个 Shiyu 区域、zh/en 两种详情语言；公开索引使用多实体 v3 外壳，类别
（`agents`、`drive-discs`、`w-engines`、`bangboos`、`monsters` 与 `shiyu`）与成员文件摘要来自完整验证后的
同一发布副本。驱动盘、WEngine 与邦布已进入生产快照、JSON 子路径导出及根入口的公开名称类型、名称 catalog
与读取 API；怪物与 Shiyu 已进入生产快照、JSON 子路径导出及根入口的公开 ID 类型、冻结 ID 列表与读取 API。
Monster 与 Shiyu 及后续 End Game 类别名称在类内重名或缺失，公开身份统一使用来源索引顶层 ID 的规范十进制
字符串，不套用英文名称唯一性，也不创造名称、别名或 slug。
...
`LocalizedMonster`、`monsterIds`、`ShiyuId`、`LocalizedShiyu`、`shiyuIds` 和十九个读取函数。
...
Monster 与 Shiyu 的名称是普通来源文本，占位名称与类内重名不参与目录生成，也不触发该检查。发布源必须包含
`agents`、`drive-discs`、`w-engines`、`bangboos`、`monsters` 与 `shiyu` 六个类别及完整 zh/en 详情；
`MonsterId`/`ShiyuId` 字面量 union、冻结 ID 列表与懒加载表由同一发布副本对应类别的 `memberIds` 生成。英文更名、
...
运行时冻结。`ShiyuId` 与 `shiyuIds` 同理取 `integrated/shiyu/{来源ID}` 成员。ID 字面量、冻结列表与懒加载表
...
- 代理人不加载驱动盘、WEngine、邦布、怪物或 Shiyu 文件，驱动盘不加载代理人、WEngine、邦布、怪物、
  Shiyu 文件或索引，WEngine 不加载代理人、驱动盘、邦布、怪物、Shiyu 文件或索引，邦布不加载代理人、
  驱动盘、WEngine、怪物、Shiyu 文件或索引，怪物不加载代理人、驱动盘、WEngine、邦布、Shiyu 文件或索引，
  Shiyu 不加载代理人、驱动盘、WEngine、邦布、怪物文件或索引；各类别加载边界互不串读。End Game 类别中的
...
索引成员引用为 `files.data` 与 `files.details.{locale}`。包内包含六个类别的完整数据；根入口的名称与 ID
