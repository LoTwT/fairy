...
- `V4.3` 高价值来源覆盖：已完成 `柏妮思`、`奥菲丝&「鬼火」`、`爱丽丝`、`薇薇安`、`简`、`柳`、`格莉丝`、`爱芮` 第九批
- `V4.4` assumptions refinement：已完成第九轮细化
...
- `格莉丝`
  - 影画 2：手雷命中后的电抗降低
- `爱芮`
  - 影画 1：`[异放]` 的基础异常暴击率 / 暴击伤害
  - 影画 1：基于 `finalPanel.anomalyMastery` 的额外异常暴击率
  - 影画 2：`[异放]` 无视防御
  - 影画 2：`[妄想时刻]` 下额外无视防御
- `柳`
  - 影画 1：`[洞悉]` 状态下异常精通提升
  - 影画 2：`[极性紊乱]` 额外突刺次数对应的紊乱倍率提升
  - 影画 4：`[识破]` 目标穿透率提升

当前仍明确 out-of-scope 的相关机制：

- `柏妮思` 的 `[燃点]/[余烬]` 触发链、影画 2 的施加时机与持续时间、堆层节奏缩短
- `奥菲丝&「鬼火」` 的后台自动释放与 `[蓄炎]` 循环
- `简` 的影画 6 `[狂热]` 直入与额外攻击
- `柳` 的 `[月相]` 切换积蓄效率、影画 2 的能量消耗变化与影画层数获取 / 消耗
- `爱芮` 的 `[异放]` 比例、失衡额外倍率与影画 6 的额外结算
...
截至第九批，当前 contract 下还能稳定落地的 progression-aware 异常 / 紊乱规则已经基本收口。剩余未展开项集中在：
...
这些项目若继续硬塞进 V4，会把静态快照模型重新拉回动态模拟。因此，V4 在当前 contract 下应视为“已收口”，后续若要继续提高精度，需要单独立项新的 context 或新的 damage view。
