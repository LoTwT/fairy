...
| **D-21** | AI Plugin V1.2.2 — scope + 架构原则 + i18n contract | ✅ 锁定（2026-05-16） | 8 项 lock：AI.0 thin layer / AI.1 Claude Code + Codex only (drop Cursor) / AI.2 G6 compare gate 延后到后续 plugin patch（CLI compare 可独立先落地）/ AI.3 V1.2.3 截图识别 forward-spec / AI.4 G1-G10 acceptance / U1 P1 ZZZ player primary / U2 命名 `fairy-snapshot`/`fairy-calc`/`fairy-explain` + chaining A + trigger phrases discovery / U3 tri-layer i18n + lang detect (ii) auto-detect sticky；CLI-only 计算契约不可逆。详见 [`D-21-ai-plugin.md`](D-21-ai-plugin.md) | 中 |
| **D-22** | AI Plugin V1.2.3 — Vision Recognition（截图 → BattleSnapshot draft）| ✅ 锁定（2026-05-17）| 8 项 lock：Q1 仅社区工具截图（不支持 in-game）/ Q2 工坊 + 米游社 2 layout / Q3 PII 不存 schema / S1 strict panel = totals 仅，base+bonus → draftMetadata / I1 single-image 默认 / L1 source detect + per-source layout map / C1 高置信度模型（社区工具 structured）/ F1 NL fallback；继承 D-21 §6 CLI-only + §5 tri-layer i18n。详见 [`D-22-ai-plugin-v1.2.3-vision.md`](D-22-ai-plugin-v1.2.3-vision.md) | 中 |
