...
- [`static-build-resolver-roadmap.md`](../docs/specs/static-build-resolver-roadmap.md) — 执行路线与阶段状态：当前主线已推进到 `V29`；已覆盖 source views、trigger matrix、utility views 与 unified source-entry collection，当前正在收口主 resolver summary contract
...
- [`static-build-resolver-v29.md`](../docs/specs/static-build-resolver-v29.md) — 当前阶段：已冻结主 resolver summary contract；目标是为 `ResolveStaticBuildResult` 增加稳定 `summary`，收口单场景结果的公式乘区摘要与 diagnostics / sourceNotes / unsupportedEffects 的统计语义
