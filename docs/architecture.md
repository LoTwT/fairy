...
- [`static-build-resolver-roadmap.md`](../docs/specs/static-build-resolver-roadmap.md) — 执行路线与阶段状态：当前主线已推进到 `V220`；当前已覆盖 single-build resolver、skill matrix、trigger matrix、source damage views、source utility views、unified source-entry collection，以及对应 compact helper exports。最近阶段已完成 `V205 ~ V220` 的高层 build tool contract 收口，包括 unsupported/success response、coverage-gap helper、typed panel normalization、resolved loadout helper、catalog/scenario/damage-type/utility-support helper、source-entry context helper、统一的 loadout context helper、skill-matrix context/schema helper，以及 triggered-damage context helper。
