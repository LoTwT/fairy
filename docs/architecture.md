...
- [`static-build-resolver-roadmap.md`](../docs/specs/static-build-resolver-roadmap.md) — 执行路线与阶段状态：当前主线已推进到 `V30`；已覆盖 source views、trigger matrix、utility views、unified source-entry collection 与 main resolver summary，当前正在收口 core skill matrix summary
...
- [`static-build-resolver-v30.md`](../docs/specs/static-build-resolver-v30.md) — 当前阶段：已冻结 core skill matrix summary contract；目标是为 `ResolveStaticBuildSkillMatrixResult` 增加稳定 `summary`，把当前只存在于高层 tool 的矩阵摘要下沉到 `zzz-data`
