...
- `V412` game-modes deadly-assault list contracts：当前阶段已收口；`game-modes.ts` 中 `Deadly Assault` 的匿名 list contract 已统一复用显式 alias
- `V413` game-modes shiyu-defense enemy and wave list contracts：当前阶段已收口；`game-modes.ts` 中 `Shiyu Defense` 的 `enemy/wave` 匿名 list contract 已统一复用显式 alias
...
- `V418` game-modes threshold-simulation version and mode list contracts：当前阶段已收口；`game-modes.ts` 中 `Threshold Simulation` 的 `version/mode` 匿名 list contract 已统一复用显式 alias
- `V419` cleaned encounter and DA buff text list contracts：当前阶段已收口；`cleaned/types.ts` 中候选名、弱点、抗性与 `DA buff` 名字列表已统一复用显式 alias
...
- `V476` resolve-build resolved snapshot object contracts：当前阶段已收口；`resolve-build-schemas.ts` 中 `resolvedSnapshot` 的匿名嵌套对象已统一复用显式 interface
- `V477` resolve-build effect override contracts：当前阶段已收口；`resolve-build-schemas.ts` 中 `effectOverrides[]` 的匿名对象已统一复用显式 interface 与共享 schema
- `V478` resolve-build dynamic snapshot nested schemas：当前阶段已收口；`resolve-build-schemas.ts` 中 `dynamicSnapshot` 的匿名 nested Zod schema 已统一复用共享 schema 常量
- `V479` resolve-build state snapshot nested schemas：当前阶段已收口；`resolve-build-schemas.ts` 中 `stateSnapshot` 的匿名 nested Zod schema 已统一复用共享 schema 常量
...
- `V487` resolve-build state snapshot scalar schemas：当前阶段已收口；`resolve-build-schemas.ts` 中 `stateSnapshot` 的 leaf scalar schema 已统一复用共享 schema 常量
- `V488` resolve-build resolved snapshot scalar schemas：当前阶段已收口；`resolve-build-schemas.ts` 中 `resolvedSnapshot` 的 delta/factor leaf scalar schema 已统一复用共享 schema 常量
- `V489` resolve-build drive-disc set scalar schemas：当前阶段已收口；`resolve-build-schemas.ts` 中 `driveDiscSet` 的 leaf schema 已统一复用共享 schema 常量
