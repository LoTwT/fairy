...
import { calculateFinalStat } from "@randomplay/core"
import {
  evaluateEffects,
  parseEffectRuleSet,
  prepareEffects,
...
function prepareStarter(bindings: readonly SourceBinding[]): PreparedEffects {
  const parsed = parseEffectRuleSet(starterDefinitions)
  expect(parsed.ok).toBe(true)
  if (!parsed.ok) {
    throw new Error("starter definitions must parse")
  }
  const prepared = prepareEffects(parsed.value, bindings)
...
describe("starter definitions cross-package acceptance", () => {
  ...
  for (const setPieces of [0, 1, 2, 4]) {
    it(`woodpecker two-piece critical rate at ${setPieces} pieces`, () => {
      ...
      const twoPiece = result.value.contributions.filter(
        (contribution) =>
          contribution.origin.effectId === "disc:31000:two-piece:critical-rate",
      )
