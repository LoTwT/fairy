...
7. **格式化输出**
  ...
  - 如果走 resolveBuildSourceEntries，单独输出“额外来源条目”小节；优先使用 \`collection.summary.isUtilityOnly\`、\`collection.summary.groups\`、\`collection.summary.sourceDamageRequirementSummary\`、\`collection.summary.sourceUtilityRequirementSummary\`、\`collection.assumptionSummary\` 和 \`entry.metadata.canonicalLabel\`、\`entry.metadata.stableKey\`、\`entry.metadata.entryKind\`；按组拆 section 时，优先使用 \`collection.summary.groups[*].sourceDamageRequirementSummary\` / \`collection.summary.groups[*].sourceUtilityRequirementSummary\` / \`collection.summary.groups[*].diagnosticSummary\` / \`collection.summary.groups[*].sourceNoteSummary\` / \`collection.summary.groups[*].assumptionSummary\` 生成组内解释，不要继续手工统计 source-damage-view / source-utility-view 数量，也不要自己遍历 mixed entries 重算 requirement / diagnostics / source-notes 分布；如果只想先判断整组 mixed collection 是否带 assumptions，也优先读取 \`collection.assumptionSummary\`；如果只想先判断某一组是否带 assumptions，也优先读取 \`collection.summary.groups[*].assumptionSummary\`；不要把 utility 条目并回主公式，也不要把 normal / sheer 场景下的 utility-only collection 误写成 source damage 列表
  ...
  - 如果走 resolveBuildSourceUtilityViews，单独输出“回能 / utility 条目”小节，优先使用 \`views.summary.groups\`、\`views.summary.triggerCount\`、\`views.summary.rateCount\`、\`views.summary.requirementSummary\`、\`views.assumptionSummary\` 与 \`entry.metadata.canonicalLabel\`、\`entry.metadata.stableKey\`、\`entry.metadata.entryKind\`；按组拆 section 时，优先使用 \`views.summary.groups[*].requirementSummary\` / \`views.summary.groups[*].diagnosticSummary\` / \`views.summary.groups[*].sourceNoteSummary\` / \`views.summary.groups[*].assumptionSummary\` 组织组内解释；同时列出来源、类型（每次触发 / 每秒回能）、目标、数值、单位，并优先使用 \`entry.requirementSummary\` 组织触发条件 / 适用条件 / 冷却；如果只想先判断整组 utility views 是否带 assumptions，也优先读取 \`views.assumptionSummary\`；如果只想先判断某一组是否带 assumptions，也优先读取 \`views.summary.groups[*].assumptionSummary\`；如果只想先判断单条 utility entry 是否带 assumptions，也优先读取 \`entry.assumptionSummary\`；只有需要逐条展开时，再回退读取 \`entry.requirements\`、\`triggerLabel\`、\`conditionLabel\` 与 \`cooldownSeconds\`；不要把这些条目伪装成主伤害乘区，也不要自己重新统计 trigger / rate 数量
