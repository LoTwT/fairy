...
7. **格式化输出**
  ...
  - 如果走 resolveBuildSourceEntries，单独输出“额外来源条目”小节；优先使用 \`collection.summary.isUtilityOnly\`、\`collection.summary.groups\`、\`collection.summary.sourceDamageRequirementSummary\`、\`collection.summary.sourceUtilityRequirementSummary\`、\`collection.summary.assumptionSummary\`、\`collection.assumptionSummary\` 和 \`entry.metadata.canonicalLabel\`、\`entry.metadata.stableKey\`、\`entry.metadata.entryKind\`；按组拆 section 时，优先使用 \`collection.summary.groups[*].sourceDamageRequirementSummary\` / \`collection.summary.groups[*].sourceUtilityRequirementSummary\` / \`collection.summary.groups[*].diagnosticSummary\` / \`collection.summary.groups[*].sourceNoteSummary\` / \`collection.summary.groups[*].assumptionSummary\` 生成组内解释，不要继续手工统计 source-damage-view / source-utility-view 数量，也不要自己遍历 mixed entries 重算 requirement / diagnostics / source-notes 分布；如果只想先判断整组 mixed collection 是否带 assumptions，也优先读取 \`collection.summary.assumptionSummary\`；如需兼容旧调用方，也可以继续读取 \`collection.assumptionSummary\`；如果只想先判断某一组是否带 assumptions，也优先读取 \`collection.summary.groups[*].assumptionSummary\`；不要把 utility 条目并回主公式，也不要把 normal / sheer 场景下的 utility-only collection 误写成 source damage 列表
