...
7. **格式化输出**
   - 如果走 resolveBuildTriggerMatrix，单独输出“触发条目矩阵”小节，优先使用 \`matrix.summary.hasSourceViews\`、\`matrix.summary.groups\`、\`matrix.summary.requirementSummary\` 与 \`row.metadata.canonicalLabel\`、\`row.metadata.stableKey\`、\`row.metadata.entryKind\`；source-view 行要带上 requirements / diagnostics / sourceNotes / assumptions，其中 requirements 优先使用结构化 \`matrix.summary.requirementSummary\` / \`row.requirementSummary\`，不要继续手工拼接成技能表，也不要自己重新统计主公式 / source-view 数量
   - 如果走 resolveBuildSourceEntries，单独输出“额外来源条目”小节；优先使用 \`collection.summary.isUtilityOnly\`、\`collection.summary.groups\`、\`collection.summary.sourceDamageRequirementSummary\`、\`collection.summary.sourceUtilityRequirementSummary\` 和 \`entry.metadata.canonicalLabel\`、\`entry.metadata.stableKey\`、\`entry.metadata.entryKind\`，不要继续手工统计 source-damage-view / source-utility-view 数量，也不要自己遍历 mixed entries 重算 requirement 分布；不要把 utility 条目并回主公式，也不要把 normal / sheer 场景下的 utility-only collection 误写成 source damage 列表
  - 如果走 resolveBuildSourceDamageViews，单独输出“额外结算条目”小节，优先使用 \`views.summary.groups\`、\`views.summary.standaloneCount\`、\`views.summary.deltaCount\`、\`views.summary.requirementSummary\` 与 \`entry.metadata.canonicalLabel\`、\`entry.metadata.stableKey\`、\`entry.metadata.entryKind\`；同时列出来源、模式（standalone / delta）、当前期望 / 暴击 / 非暴击，以及 requirements / diagnostics / sourceNotes / assumptions；优先使用结构化 diagnostics + sourceNotes 说明默认值、coverage gap、缺少输入、已展开或 research-only，不要继续手工拆 assumptions 字符串，也不要自己重新统计 standalone / delta 数量；不要把这些条目直接并入主伤害表或矩阵
  - 如果走 resolveBuildSourceUtilityViews，单独输出“回能 / utility 条目”小节，优先使用 \`views.summary.groups\`、\`views.summary.triggerCount\`、\`views.summary.rateCount\`、\`views.summary.requirementSummary\` 与 \`entry.metadata.canonicalLabel\`、\`entry.metadata.stableKey\`、\`entry.metadata.entryKind\`；同时列出来源、类型（每次触发 / 每秒回能）、目标、数值、单位，并优先使用 \`entry.requirementSummary\` 组织触发条件 / 适用条件 / 冷却；只有需要逐条展开时，再回退读取 \`entry.requirements\`、\`triggerLabel\`、\`conditionLabel\` 与 \`cooldownSeconds\`；不要把这些条目伪装成主伤害乘区，也不要自己重新统计 trigger / rate 数量
