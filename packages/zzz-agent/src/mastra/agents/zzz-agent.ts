...
7. **格式化输出**
  ...
  - 如果走 resolveBuildSourceUtilityViews，单独输出“回能 / utility 条目”小节，优先使用 \`views.summary.groups\`、\`views.summary.triggerCount\`、\`views.summary.rateCount\`、\`views.summary.requirementSummary\`、\`views.summary.assumptionSummary\`、\`views.summary.caveatSummary\`、\`views.assumptionSummary\`、\`views.caveatSummary\` 与 \`entry.metadata.canonicalLabel\`、\`entry.metadata.stableKey\`、\`entry.metadata.entryKind\`；按组拆 section 时，优先使用 \`views.summary.groups[*].requirementSummary\` / \`views.summary.groups[*].diagnosticSummary\` / \`views.summary.groups[*].sourceNoteSummary\` / \`views.summary.groups[*].assumptionSummary\` 组织组内解释；同时列出来源、类型（每次触发 / 每秒回能）、目标、数值、单位，并优先使用 \`entry.requirementSummary\` 组织触发条件 / 适用条件 / 冷却；如果只想先判断整组 utility views 是否带 caveat，也优先读取 \`views.summary.caveatSummary\`；如需兼容旧调用方，也可以继续读取 \`views.caveatSummary\`；如果只想先判断整组 utility views 是否带 assumptions，也优先读取 \`views.summary.assumptionSummary\`；如需兼容旧调用方，也可以继续读取 \`views.assumptionSummary\`；如果只想先判断某一组是否带 assumptions，也优先读取 \`views.summary.groups[*].assumptionSummary\`；如果只想先判断单条 utility entry 是否带 assumptions，也优先读取 \`entry.assumptionSummary\`；只有需要逐条展开时，再回退读取 \`entry.requirements\`、\`triggerLabel\`、\`conditionLabel\` 与 \`cooldownSeconds\`；不要把这些条目伪装成主伤害乘区，也不要自己重新统计 trigger / rate 数量
