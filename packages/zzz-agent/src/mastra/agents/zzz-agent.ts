...
7. **格式化输出**
  ...
  - 如果走 resolveBuildSourceUtilityViews，单独输出“回能 / utility 条目”小节，优先使用 \`views.summary.groups\`、\`views.summary.triggerCount\`、\`views.summary.rateCount\` 与 \`entry.metadata.canonicalLabel\`、\`entry.metadata.stableKey\`、\`entry.metadata.entryKind\`；同时列出来源、类型（每次触发 / 每秒回能）、目标、数值、单位，并优先使用 \`entry.requirementSummary\` 组织触发条件 / 适用条件 / 冷却；只有需要逐条展开时，再回退读取 \`entry.requirements\`、\`triggerLabel\`、\`conditionLabel\` 与 \`cooldownSeconds\`；不要把这些条目伪装成主伤害乘区，也不要自己重新统计 trigger / rate 数量
