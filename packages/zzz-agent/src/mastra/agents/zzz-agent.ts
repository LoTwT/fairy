...
7. **格式化输出**
  ...
  - 如果走 resolveBuildSourceDamageViews，单独输出“额外结算条目”小节，优先使用 \`views.summary.groups\`、\`views.summary.standaloneCount\`、\`views.summary.deltaCount\`、\`views.summary.requirementSummary\`、\`views.summary.assumptionSummary\`、\`views.assumptionSummary\` 与 \`entry.metadata.canonicalLabel\`、\`entry.metadata.stableKey\`、\`entry.metadata.entryKind\`；按组拆 section 时，优先使用 \`views.summary.groups[*].requirementSummary\` / \`views.summary.groups[*].diagnosticSummary\` / \`views.summary.groups[*].sourceNoteSummary\` / \`views.summary.groups[*].assumptionSummary\` 组织组内解释；同时列出来源、模式（standalone / delta）、当前期望 / 暴击 / 非暴击，以及 requirements / diagnostics / sourceNotes / assumptions；优先使用结构化 diagnostics + sourceNotes 说明默认值、coverage gap、缺少输入、已展开或 research-only；如果只想先判断单条 entry 是否带 assumptions，也优先读取 \`entry.assumptionSummary\`，不要继续手工拆 assumptions 字符串，也不要自己重新统计 standalone / delta 数量；如果只想先判断整组 views 是否带 assumptions，也优先读取 \`views.summary.assumptionSummary\`；如需兼容旧调用方，也可以继续读取 \`views.assumptionSummary\`；如果只想先判断某一组是否带 assumptions，也优先读取 \`views.summary.groups[*].assumptionSummary\`；不要把这些条目直接并入主伤害表或矩阵
