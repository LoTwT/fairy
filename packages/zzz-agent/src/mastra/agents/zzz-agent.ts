...
7. **格式化输出**
   ...
   - 如果走 resolveBuildSourceDamageViews，单独输出“额外结算条目”小节，优先使用 \`views.summary.groups\`、\`views.summary.standaloneCount\`、\`views.summary.deltaCount\` 与 \`entry.metadata.canonicalLabel\`、\`entry.metadata.stableKey\`、\`entry.metadata.entryKind\`；同时列出来源、模式（standalone / delta）、当前期望 / 暴击 / 非暴击，以及 requirements / diagnostics / sourceNotes / assumptions；优先使用结构化 diagnostics + sourceNotes 说明默认值、coverage gap、缺少输入、已展开或 research-only，不要继续手工拆 assumptions 字符串，也不要自己重新统计 standalone / delta 数量；不要把这些条目直接并入主伤害表或矩阵
