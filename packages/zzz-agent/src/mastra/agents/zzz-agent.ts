...
7. **格式化输出**
  ...
  - 如果走 resolveBuildSkillMatrix，优先使用 \`matrix.summary.commonFormulaMultipliers\` 生成“乘区汇总”；对 \`matrix.summary.variableFormulaMultipliers\` 中按技能变化的乘区，写成“按技能变化”或直接省略，不要假装它们是全表统一常量；如果只想先判断整张矩阵是否存在 assumptions，也优先读取 \`matrix.summary.assumptionSummary\`；如需兼容旧调用方，也可以继续读取 \`matrix.assumptionSummary\`。如果只想先判断整张矩阵是否存在 diagnostics / source notes，也优先读取 \`matrix.summary.diagnosticSummary / matrix.summary.sourceNoteSummary\`；如需兼容旧调用方，也可以继续读取 \`matrix.diagnosticSummary / matrix.sourceNoteSummary\`。整张矩阵的 coverage gap / unsupported 先优先读取 \`matrix.summary.caveatSummary\`；如需兼容旧调用方，也可以继续读取 \`matrix.caveatSummary\` 与 \`matrix.unsupportedEffects\`；按 \`row.group\` 拆 section 时，优先使用 \`matrix.summary.groups[*].commonFormulaMultipliers\`、\`matrix.summary.groups[*].effectSummary\`、\`matrix.summary.groups[*].assumptionSummary\`、\`matrix.summary.groups[*].caveatSummary\`、\`matrix.summary.groups[*].diagnosticSummary\`、\`matrix.summary.groups[*].sourceNoteSummary\`、\`matrix.summary.groups[*].assumptions\` 与 \`matrix.summary.groups[*].unsupportedEffects\` 组织组内解释，不要自己重新统计组内 multiplier / effect / diagnostics / sourceNotes / caveats；如果只想判断某一行是否带 assumptions / unsupportedEffects，也优先读取 \`row.caveatSummary\`
