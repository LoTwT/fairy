...
function snapshotAssets(snapshot) {
  ...
  const monsterAssets = [
    {
      id: "monster-index",
      url: `https://static.nanoka.cc/zzz/${snapshot}/monster.json`,
      path: "monster.json",
      entityType: "monsterIndex",
      approvedForCleanedOutput: true,
      evidenceUse: "v1.2.x-enemy-batch-source-gate",
    },
    ...monsterEntityIds.map(entityId => ({
      id: `monster-${entityId}`,
      url: `https://static.nanoka.cc/zzz/${snapshot}/zh/monster/${entityId}.json`,
      path: `zh/monster/${entityId}.json`,
      entityType: "monster",
      language: "zh",
      entityId,
      approvedForCleanedOutput: true,
...
function summarizeSnapshot(snapshot, assets) {
  ...
  const monsterDetails = monsterEntityIds.map((entityId) => {
    ...
    if (indexEntry === undefined)
      throw new Error(`monster index is missing ${entityId}`)
    const detail = readJson(join(sourceRoot, snapshot, `zh/monster/${entityId}.json`))
    if (detail.id !== entityId)
      throw new Error(`monster ${entityId}: detail id drifted`)
    if (detail.name !== indexEntry.zh)
      throw new Error(`monster ${entityId}: zh name drifted against index`)
    return { indexEntry, detail }
  })
  ...
  if (Object.keys(monsterIndex).length !== monsterEntityIds.length)
    throw new Error(`monster index count drifted: expected ${monsterEntityIds.length}, got ${Object.keys(monsterIndex).length}`)
  ...
  return {
    ...
    enemyBatch: {
      indexCount: Object.keys(monsterIndex).length,
      retainedDetailCount: monsterDetails.length,
      selectedVariantCount: monsterDetails.filter(({ detail }) => detail.monster_info?.[String(detail.monster_id)] !== undefined).length,
      missingSelectedVariantCount: monsterDetails.filter(({ detail }) => detail.monster_info?.[String(detail.monster_id)] === undefined).length,
      skippedVariantCount: monsterDetails.reduce((total, { detail }) => {
        const variantCount = Object.keys(detail.monster_info ?? {}).length
        return total + (detail.monster_info?.[String(detail.monster_id)] === undefined ? variantCount : Math.max(variantCount - 1, 0))
      }, 0),
      ids: monsterDetails.map(({ detail }) => detail.id),
      approvedForCleanedOutputCount: assets.filter(asset => asset.entityType === "monster" && asset.approvedForCleanedOutput === true).length,
    },
    bangbooSample: {
      id: bangboo.id,
  ...
  const manifest = {
    ...
    urlPolicy: {
      ...
      approvedIndexUrls: [
        ...
        `https://static.nanoka.cc/zzz/${snapshot}/monster.json`,
      ],
...
function verifySnapshot(snapshot) {
  ...
  if (summary.enemyBatch?.indexCount !== manifest.summary?.enemyBatch?.indexCount)
    throw new Error("enemy index count summary drifted")
  if (summary.enemyBatch?.retainedDetailCount !== manifest.summary?.enemyBatch?.retainedDetailCount)
    throw new Error("enemy retained detail count summary drifted")
  if (summary.enemyBatch?.selectedVariantCount !== manifest.summary?.enemyBatch?.selectedVariantCount)
    throw new Error("enemy selected variant count summary drifted")
  if (summary.enemyBatch?.missingSelectedVariantCount !== manifest.summary?.enemyBatch?.missingSelectedVariantCount)
    throw new Error("enemy missing selected variant count summary drifted")
  if (summary.enemyBatch?.skippedVariantCount !== manifest.summary?.enemyBatch?.skippedVariantCount)
    throw new Error("enemy skipped variant count summary drifted")
  if (JSON.stringify(summary.enemyBatch?.ids) !== JSON.stringify(manifest.summary?.enemyBatch?.ids))
    throw new Error("enemy retained detail ids summary drifted")
