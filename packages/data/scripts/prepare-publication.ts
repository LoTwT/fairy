...
/**
 * 本次发布全部 Boss 试炼的来源索引顶层 ID（规范十进制字符串）。
...
export const agentActionLoaders: Record<string, () => Promise<AgentActions>> = {
${agents.memberIds.map((id) => `${json(id)}: ${lazyActions(id)}`).join(",\n")}
}
