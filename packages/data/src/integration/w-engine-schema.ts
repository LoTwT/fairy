 ...
 *
 * 字段集合依据来源说明与本地 3.1 `weapon.json` 的 95 条记录确认：索引是 W-Engine ID → 记录的对象，
 * 记录内嵌各语言名称与展示摘要。登记不要求这些字段存在、不校验其类型，也不改变 `sourceRecord` 的
 * 原 key 与原值；索引与详情是两个独立来源，同名字段不要求相等，也不互相回退。
 * 未登记字段内部不递归推断字段身份。
 */
export const wEngineIndexFields: readonly string[] = [
  /** 来源 `atk`。索引内嵌的基础攻击力数值；与详情属性值分开保留，不换算或比较。 */
  "atk",

  /** 来源 `desc`。索引内嵌的说明文本；不当作某语言详情或回退文本。 */
  "desc",

  /** 来源 `en`。索引内嵌的英文名称；不能冒充完整详情。 */
  "en",

  /** 来源 `icon`。索引内嵌的图标编码（如 Weapon_B_Common_01）；与详情 icon 的资源路径分开保留。 */
  "icon",
  ...
  /** 来源 `sub`。副属性显示名（如 ATK、CRIT Rate）；来源用词原样保留。 */
  "sub",

  /** 来源 `type`。分类 ID；与详情 weapon_type 的 key 空间一致，不映射为 core 枚举。 */
  "type",
