...
function validateGeneralStatInput(
  ...
  let valid = true
  const settled = object["settledInitialValue"] !== undefined
  for (const field of settled
    ? [
        "settledInitialValue",
        ...(object["baseValue"] === undefined ? [] : ["baseValue"]),
      ]
    : ["baseValue"]) {
    if (
  ...
  for (const field of [
    ...(settled ? [] : ["initialPercentage", "initialFixed"]),
    "finalPercentage",
    "finalFixed",
  ] as const) {
  ...
  rejectUnknownFields(
    ...
    [
      ...
      ...(settled
        ? ["settledInitialValue"]
        : ["initialPercentage", "initialFixed"]),
      "finalPercentage",
      "finalFixed",
    ],
    checks,
