...
原 effects 包的校验、状态、求值与静态目录入口均从本包导出。

Node 要求 `>=24.11.0`，使用 ESM；浏览器已验收 Vite 开发与生产构建。
配合 data 使用时须安装同一版本，底层公式也可单独使用本包。
[安装与旧版升级](https://github.com/LoTwT/fairy/blob/main/docs/specs/packages.md)说明当前源码 API 与 npm `0.1.4` 的差异。
[完整静态计算示例](https://github.com/LoTwT/fairy/blob/main/docs/specs/core/static-calculation.md#完整计算示例)
包含配装、局外面板、技能等级、显式 buff、目标、错误处理和结果解释。
