...
- `compactStaticBuildSourceEntryCollection(collection)` 默认不带 mixed `entry.requirements / entry.diagnostics / entry.sourceNotes / entry.build`；只有 `compactStaticBuildSourceEntryCollection(collection, true)` 才会展开这些明细，其中 `entry.build` 仅在对应 `source-damage-view` 原始结果本来带 `build` 时透传
