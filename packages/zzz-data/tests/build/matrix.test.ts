...
describe("static build skill matrix", () => {
  it("builds Zhu Yuan full skill matrix rows", () => {
    ...
    expect(result.summary.caveatSummary).toEqual(result.caveatSummary)
    ...
    expect(result.summary.assumptionSummary).toEqual(result.assumptionSummary)
    ...
    expect(etherBurst?.summary.expectedTotal).toBeCloseTo(
      etherBurst?.damageSummary.expected ?? 0,
      6,
    )
    expect(etherBurst?.summary.formulaMultipliers).toBeTruthy()
    expect(etherBurst?.diagnostics).toBeInstanceOf(Array)
    expect(etherBurst?.diagnosticSummary.count).toBe(
      etherBurst?.diagnostics.length ?? 0,
    )
    expect(etherBurst?.sourceNotes).toBeInstanceOf(Array)
    expect(etherBurst?.sourceNoteSummary.count).toBe(
      etherBurst?.sourceNotes.length ?? 0,
    )
  ...
  it("builds Yixuan skill matrix with sheer rows", () => {
    ...
    expect(ultimate?.summary.expectedTotal).toBeCloseTo(
      ultimate?.damageSummary.expected ?? 0,
      6,
    )
    expect(ultimate?.diagnosticSummary.count).toBe(
      ultimate?.diagnostics.length ?? 0,
    )
    expect(ultimate?.sourceNoteSummary.count).toBe(
      ultimate?.sourceNotes.length ?? 0,
    )
  ...
  it("propagates Ye Shunguang curtain snapshot through skill matrix context", () => {
    const result = resolveStaticBuildSkillMatrix({
      mode: "full-buff",
      loadout: {
        agentId: "1431",
